...
?DOC(false).
-spec try_or({ok, KUG} | {error, any()}, KUK, fun((KUG) -> KUK)) -> KUK.
...
-spec try_fold_clauses(
    list(glance:clause()),
    KTT,
    fun((KTT, glance:expression()) -> {ok, KTT} | {error, KTU})
) -> {ok, KTT} | {error, KTU}.
...
-spec try_fold_statements(
    ...
    KSY,
    fun((KSY, glance:expression()) -> {ok, KSY} | {error, KSZ})
) -> {ok, KSY} | {error, KSZ}.
...
-spec try_fold_fields(
    ...
    KTM,
    fun((KTM, glance:expression()) -> {ok, KTM} | {error, KTN})
) -> {ok, KTM} | {error, KTN}.
...
-spec try_fold_expressions(
    ...
    KUA,
    fun((KUA, glance:expression()) -> {ok, KUA} | {error, KUB})
) -> {ok, KUA} | {error, KUB}.
