...
?DOC(false).
-spec try_or({ok, KSD} | {error, any()}, KSH, fun((KSD) -> KSH)) -> KSH.
...
-spec try_fold_expression(
    glance:expression(),
    KRB,
    fun((KRB, glance:expression()) -> {ok, KRB} | {error, KRC})
) -> {ok, KRB} | {error, KRC}.
...
-spec try_fold_clauses(
    list(glance:clause()),
    KRQ,
    fun((KRQ, glance:expression()) -> {ok, KRQ} | {error, KRR})
) -> {ok, KRQ} | {error, KRR}.
...
-spec try_fold_statements(
    ...
    KQV,
    fun((KQV, glance:expression()) -> {ok, KQV} | {error, KQW})
) -> {ok, KQV} | {error, KQW}.
...
-spec try_fold_fields(
    ...
    KRJ,
    fun((KRJ, glance:expression()) -> {ok, KRJ} | {error, KRK})
) -> {ok, KRJ} | {error, KRK}.
...
-spec try_fold_expressions(
    ...
    KRX,
    fun((KRX, glance:expression()) -> {ok, KRX} | {error, KRY})
) -> {ok, KRX} | {error, KRY}.
