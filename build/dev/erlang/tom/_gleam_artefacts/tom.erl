...
-spec push_key({ok, HYF} | {error, get_error()}, binary()) -> {ok, HYF} |
    {error, get_error()}.
push_key(Result, Key) ->
    case Result of
        {ok, T} ->
...
-spec expect_end_of_line(
    list(binary()),
    fun((list(binary())) -> {ok, {IAK, list(binary())}} | {error, parse_error()})
) -> {ok, {IAK, list(binary())}} | {error, parse_error()}.
...
-file("src/tom.gleam", 758).
-spec do(
    {ok, {IAV, list(binary())}} | {error, parse_error()},
    fun((IAV, list(binary())) -> {ok, IAY} | {error, parse_error()})
) -> {ok, IAY} | {error, parse_error()}.
do(Result, Next) ->
    case Result of
        {ok, {A, Input}} ->
            Next(A, Input);
...
-file("src/tom.gleam", 768).
-spec expect(
    list(binary()),
    binary(),
    fun((list(binary())) -> {ok, {IBD, list(binary())}} | {error, parse_error()})
) -> {ok, {IBD, list(binary())}} | {error, parse_error()}.
