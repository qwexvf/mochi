...
-spec parse_bits(bitstring(), gleam@dynamic@decode:decoder(GIM)) -> {ok, GIM} |
    {error, decode_error()}.
...
-spec do_parse(binary(), gleam@dynamic@decode:decoder(GIG)) -> {ok, GIG} |
    {error, decode_error()}.
...
-spec parse(binary(), gleam@dynamic@decode:decoder(GIC)) -> {ok, GIC} |
    {error, decode_error()}.
parse(Json, Decoder) ->
    do_parse(Json, Decoder).
...
-spec nullable(gleam@option:option(GIS), fun((GIS) -> json())) -> json().
...
-spec array(list(GIW), fun((GIW) -> json())) -> json().
