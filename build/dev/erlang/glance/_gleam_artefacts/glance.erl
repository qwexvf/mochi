...
-type definition(DWY) :: {definition, list(attribute()), DWY}.
...
-type bit_string_segment_option(DWZ) :: bytes_option |
    int_option |
    float_option |
    bits_option |
    utf8_option |
...
-type record_update_field(DXA) :: {record_update_field,
        binary(),
        gleam@option:option(DXA)}.

-type variant_field() :: {labelled_variant_field, type(), binary()} |
    {unlabelled_variant_field, type()}.

-type field(DXB) :: {labelled_field, binary(), span(), DXB} |
    {shorthand_field, binary(), span()} |
    {unlabelled_field, DXB}.
...
-type parsed_list(DXC) :: {parsed_list,
        list(DXC),
        gleam@option:option(DXC),
        list({glexer@token:token(), glexer:position()}),
        integer()}.
...
-spec expect_name(
    list({glexer@token:token(), glexer:position()}),
    fun((binary(), list({glexer@token:token(), glexer:position()})) -> {ok, DXU} |
        {error, error()})
) -> {ok, DXU} | {error, error()}.
...
-spec until(
    ...
    DXZ,
    list({glexer@token:token(), glexer:position()}),
    fun((DXZ, list({glexer@token:token(), glexer:position()})) -> {ok,
            {DXZ, list({glexer@token:token(), glexer:position()})}} |
        {error, error()})
) -> {ok, {DXZ, integer(), list({glexer@token:token(), glexer:position()})}} |
    {error, error()}.
...
-spec list(
    fun((list({glexer@token:token(), glexer:position()})) -> {ok,
            {EDS, list({glexer@token:token(), glexer:position()})}} |
        {error, error()}),
    gleam@option:option(fun((span()) -> EDS)),
    list(EDS),
    list({glexer@token:token(), glexer:position()})
) -> {ok, parsed_list(EDS)} | {error, error()}.
...
-spec bit_string_segment_options(
    fun((list({glexer@token:token(), glexer:position()})) -> {ok,
            {EBP, list({glexer@token:token(), glexer:position()})}} |
        {error, error()}),
    list(bit_string_segment_option(EBP)),
    list({glexer@token:token(), glexer:position()})
) -> {ok,
        {list(bit_string_segment_option(EBP)),
            list({glexer@token:token(), glexer:position()})}} |
...
-spec optional_bit_string_segment_options(
    fun((list({glexer@token:token(), glexer:position()})) -> {ok,
            {EBI, list({glexer@token:token(), glexer:position()})}} |
        {error, error()}),
    list({glexer@token:token(), glexer:position()})
) -> {ok,
        {list(bit_string_segment_option(EBI)),
            list({glexer@token:token(), glexer:position()})}} |
    {error, error()}.
...
-spec bit_string_segment(
    fun((list({glexer@token:token(), glexer:position()})) -> {ok,
            {EBB, list({glexer@token:token(), glexer:position()})}} |
        {error, error()}),
    list({glexer@token:token(), glexer:position()})
) -> {ok,
        {{EBB, list(bit_string_segment_option(EBB))},
            list({glexer@token:token(), glexer:position()})}} |
    {error, error()}.
...
-spec delimited(
    ...
    list({glexer@token:token(), glexer:position()}),
    fun((list({glexer@token:token(), glexer:position()})) -> {ok,
            {EDI, list({glexer@token:token(), glexer:position()})}} |
        {error, error()}),
    glexer@token:token()
) -> {ok, {list(EDI), list({glexer@token:token(), glexer:position()})}} |
    {error, error()}.
...
-spec comma_delimited(
    ...
    list({glexer@token:token(), glexer:position()}),
    fun((list({glexer@token:token(), glexer:position()})) -> {ok,
            {EEK, list({glexer@token:token(), glexer:position()})}} |
        {error, error()}),
    glexer@token:token()
) -> {ok,
        {list(EEK), integer(), list({glexer@token:token(), glexer:position()})}} |
    {error, error()}.
...
-spec field(
    list({glexer@token:token(), glexer:position()}),
    fun((list({glexer@token:token(), glexer:position()})) -> {ok,
            {EFV, list({glexer@token:token(), glexer:position()})}} |
        {error, error()})
) -> {ok, {field(EFV), list({glexer@token:token(), glexer:position()})}} |
    {error, error()}.
field(Tokens, Parser) ->
    case Tokens of
