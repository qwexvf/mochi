...
-opaque trie(JQI, JQJ) :: {trie,
        gleam@option:option(JQJ),
        gleam@dict:dict(JQI, trie(JQI, JQJ))}.
...
-spec do_delete(trie(JQR, JQS), list(JQR)) -> gleam@option:option(trie(JQR, JQS)).
do_delete(Trie, Path) ->
    case {Path, Trie} of
        {[], {trie, _, Children_map}} ->
...
-spec fold(trie(JQZ, JRA), JRD, fun((JRD, list(JQZ), JRA) -> JRD)) -> JRD.
...
-spec get(trie(JRL, JRM), list(JRL)) -> {ok, JRM} | {error, nil}.
get(From, Path) ->
    case {Path, From} of
        {[], {trie, none, _}} ->
            {error, nil};
...
-spec has_path(trie(JRS, any()), list(JRS)) -> boolean().
has_path(Trie, Path) ->
...
-spec map(trie(JSI, JSJ), fun((JSJ) -> JSM)) -> trie(JSI, JSM).
map(Trie, Fun) ->
    {trie,
...
-spec delete(trie(JQK, JQL), list(JQK)) -> trie(JQK, JQL).
delete(Trie, Path) ->
...
-spec insert(trie(JRX, JRY), list(JRX), JRY) -> trie(JRX, JRY).
...
-spec from_list(list({list(JRF), JRH})) -> trie(JRF, JRH).
from_list(List) ->
    gleam@list:fold(
        List,
        new(),
...
-spec paths(trie(JST, any())) -> list(list(JST)).
paths(Trie) ->
...
-spec singleton(list(JSZ), JTB) -> trie(JSZ, JTB).
...
-spec subtrie(trie(JTI, JTJ), list(JTI)) -> {ok, trie(JTI, JTJ)} | {error, nil}.
subtrie(Trie, Prefix) ->
    case {Prefix, Trie} of
        {[], _} ->
            {ok, Trie};
...
-spec to_list(trie(JTR, JTS)) -> list({list(JTR), JTS}).
to_list(Trie) ->
...
-spec do_update(
    trie(JUG, JUH),
    list(JUG),
    fun((gleam@option:option(JUH)) -> gleam@option:option(JUH))
) -> gleam@option:option(trie(JUG, JUH)).
...
-spec update(
    trie(JTX, JTY),
    list(JTX),
    fun((gleam@option:option(JTY)) -> gleam@option:option(JTY))
) -> trie(JTX, JTY).
...
-spec values(trie(any(), JUR)) -> list(JUR).
values(Trie) ->
