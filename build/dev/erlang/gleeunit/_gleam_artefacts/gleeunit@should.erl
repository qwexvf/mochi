...
-spec be_ok({ok, MRH} | {error, any()}) -> MRH.
be_ok(A) ->
    case A of
        {ok, Value} ->
            Value;
...
-spec be_error({ok, any()} | {error, MRM}) -> MRM.
be_error(A) ->
    case A of
        {error, Error} ->
            Error;
...
-spec be_some(gleam@option:option(MRP)) -> MRP.
be_some(A) ->
    case A of
        {some, Value} ->
            Value;
