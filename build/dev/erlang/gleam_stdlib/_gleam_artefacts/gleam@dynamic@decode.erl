...
-opaque decoder(BVW) :: {decoder,
        fun((gleam@dynamic:dynamic_()) -> {BVW, list(decode_error())})}.
...
-spec run(gleam@dynamic:dynamic_(), decoder(BWE)) -> {ok, BWE} |
...
-spec map(decoder(CAC), fun((CAC) -> CAE)) -> decoder(CAE).
...
-spec then(decoder(CAO), fun((CAO) -> decoder(CAQ))) -> decoder(CAQ).
then(Decoder, Next) ->
    {decoder,
        fun(Dynamic_data) ->
...
-spec one_of(decoder(CAT), list(decoder(CAT))) -> decoder(CAT).
...
-spec recursive(fun(() -> decoder(CBJ))) -> decoder(CBJ).
recursive(Inner) ->
    {decoder,
        fun(Data) ->
            Decoder = Inner(),
...
-spec optional(decoder(BZY)) -> decoder(gleam@option:option(BZY)).
optional(Inner) ->
...
-spec run_dynamic_function(
    gleam@dynamic:dynamic_(),
    binary(),
    fun((gleam@dynamic:dynamic_()) -> {ok, BXZ} | {error, BXZ})
) -> {BXZ, list(decode_error())}.
...
-spec collapse_errors(decoder(CAL), binary()) -> decoder(CAL).
...
-spec failure(CBD, binary()) -> decoder(CBD).
...
-spec new_primitive_decoder(
    binary(),
    fun((gleam@dynamic:dynamic_()) -> {ok, CBF} | {error, CBF})
) -> decoder(CBF).
...
-spec fold_dict(
    {gleam@dict:dict(BZK, BZL), list(decode_error())},
    gleam@dynamic:dynamic_(),
    gleam@dynamic:dynamic_(),
...
-spec dict(decoder(BZD), decoder(BZF)) -> decoder(gleam@dict:dict(BZD, BZF)).
dict(Key, Value) ->
...
-spec list(decoder(BYR)) -> decoder(list(BYR)).
list(Inner) ->
    {decoder,
        fun(Data) ->
            gleam_stdlib:list(
...
-spec push_path({BXA, list(decode_error())}, list(any())) -> {BXA,
    list(decode_error())}.
...
-spec index(
    list(BWO),
    list(BWO),
    fun((gleam@dynamic:dynamic_()) -> {BWR, list(decode_error())}),
    gleam@dynamic:dynamic_(),
    fun((gleam@dynamic:dynamic_(), list(BWO)) -> {BWR, list(decode_error())})
...
-spec subfield(list(any()), decoder(BVZ), fun((BVZ) -> decoder(BWB))) -> decoder(BWB).
...
-spec at(list(any()), decoder(BWL)) -> decoder(BWL).
at(Path, Inner) ->
    {decoder,
        fun(Data) ->
            index(
...
-spec field(any(), decoder(BXJ), fun((BXJ) -> decoder(BXL))) -> decoder(BXL).
...
-spec optional_field(any(), BXP, decoder(BXP), fun((BXP) -> decoder(BXR))) -> decoder(BXR).
...
-spec optionally_at(list(any()), BXW, decoder(BXW)) -> decoder(BXW).
