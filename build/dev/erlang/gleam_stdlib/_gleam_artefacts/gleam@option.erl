...
-type option(EU) :: {some, EU} | none.

-file("src/gleam/option.gleam", 57).
-spec reverse_and_prepend(list(FJ), list(FJ)) -> list(FJ).
...
-spec all_loop(list(option(FA)), list(FA)) -> option(list(FA)).
all_loop(List, Acc) ->
    case List of
        [] ->
...
-spec all(list(option(EV))) -> option(list(EV)).
all(List) ->
    all_loop(List, []).
...
-spec to_result(option(FR), FU) -> {ok, FR} | {error, FU}.
to_result(Option, E) ->
    case Option of
        {some, A} ->
            {ok, A};
...
-spec from_result({ok, FX} | {error, any()}) -> option(FX).
from_result(Result) ->
    case Result of
        {ok, A} ->
            {some, A};
...
-spec lazy_unwrap(option(GE), fun(() -> GE)) -> GE.
...
-spec map(option(GG), fun((GG) -> GI)) -> option(GI).
map(Option, Fun) ->
    case Option of
        {some, X} ->
            {some, Fun(X)};
...
-spec flatten(option(option(GK))) -> option(GK).
flatten(Option) ->
    case Option of
        {some, X} ->
            X;
...
-spec then(option(GO), fun((GO) -> option(GQ))) -> option(GQ).
then(Option, Fun) ->
    case Option of
        {some, X} ->
            Fun(X);
...
-spec 'or'(option(GT), option(GT)) -> option(GT).
'or'(First, Second) ->
    case First of
        {some, _} ->
            First;
...
-spec lazy_or(option(GX), fun(() -> option(GX))) -> option(GX).
...
-spec values_loop(list(option(HF)), list(HF)) -> list(HF).
...
-spec values(list(option(HB))) -> list(HB).
values(Options) ->
    values_loop(Options, []).
