...
-spec count_loop(list(AAU), fun((AAU) -> boolean()), integer()) -> integer().
...
-spec group(list(ABQ), fun((ABQ) -> ABS)) -> gleam@dict:dict(ABS, list(ABQ)).
group(List, Key) ->
    gleam@dict:group(Key, List).

-file("src/gleam/list.gleam", 297).
-spec filter_loop(list(ABZ), fun((ABZ) -> boolean()), list(ABZ)) -> list(ABZ).
...
-spec filter(list(ABW), fun((ABW) -> boolean())) -> list(ABW).
...
-spec map_loop(list(ACW), fun((ACW) -> ACY), list(ACY)) -> list(ACY).
...
-spec map(list(ACS), fun((ACS) -> ACU)) -> list(ACU).
map(List, Fun) ->
    map_loop(List, Fun, []).

-file("src/gleam/list.gleam", 382).
-spec map2_loop(list(ADH), list(ADJ), fun((ADH, ADJ) -> ADL), list(ADL)) -> list(ADL).
...
-spec map2(list(ADB), list(ADD), fun((ADB, ADD) -> ADF)) -> list(ADF).
map2(List1, List2, Fun) ->
    map2_loop(List1, List2, Fun, []).

-file("src/gleam/list.gleam", 416).
-spec map_fold_loop(list(ADT), fun((ADV, ADT) -> {ADV, ADW}), ADV, list(ADW)) -> {ADV,
    list(ADW)}.
...
-spec map_fold(list(ADO), ADQ, fun((ADQ, ADO) -> {ADQ, ADR})) -> {ADQ,
    list(ADR)}.
...
-spec flatten_loop(list(list(AGE)), list(AGE)) -> list(AGE).
...
-spec flatten(list(list(AGA))) -> list(AGA).
flatten(Lists) ->
    lists:append(Lists).
...
-spec flat_map(list(AGJ), fun((AGJ) -> list(AGL))) -> list(AGL).
flat_map(List, Fun) ->
...
-spec fold(list(AGO), AGQ, fun((AGQ, AGO) -> AGQ)) -> AGQ.
...
-spec fold_right(list(AGR), AGT, fun((AGT, AGR) -> AGT)) -> AGT.
...
-spec index_fold(list(AGU), AGW, fun((AGW, AGU, integer()) -> AGW)) -> AGW.
...
-spec try_fold(list(AHA), AHC, fun((AHC, AHA) -> {ok, AHC} | {error, AHD})) -> {ok,
        AHC} |
    {error, AHD}.
...
-spec fold_until(list(AHI), AHK, fun((AHK, AHI) -> continue_or_stop(AHK))) -> AHK.
...
-spec find(list(AHM), fun((AHM) -> boolean())) -> {ok, AHM} | {error, nil}.
...
-spec find_map(list(AHQ), fun((AHQ) -> {ok, AHS} | {error, any()})) -> {ok, AHS} |
    {error, nil}.
find_map(List, Fun) ->
    case List of
        [] ->
...
-spec all(list(AHY), fun((AHY) -> boolean())) -> boolean().
all(List, Predicate) ->
    case List of
        [] ->
            true;
...
-spec any(list(AIA), fun((AIA) -> boolean())) -> boolean().
any(List, Predicate) ->
    case List of
        [] ->
            false;
...
-spec zip_loop(list(AIH), list(AIJ), list({AIH, AIJ})) -> list({AIH, AIJ}).
...
-spec strict_zip_loop(list(AIU), list(AIW), list({AIU, AIW})) -> {ok,
        list({AIU, AIW})} |
    {error, nil}.
...
-spec strict_zip(list(AIN), list(AIP)) -> {ok, list({AIN, AIP})} | {error, nil}.
...
-spec unique_loop(list(AJY), gleam@dict:dict(AJY, nil), list(AJY)) -> list(AJY).
...
-spec sort(list(AKE), fun((AKE, AKE) -> gleam@order:order())) -> list(AKE).
sort(List, Compare) ->
    case List of
        [] ->
            [];
...
-spec repeat_loop(ALQ, integer(), list(ALQ)) -> list(ALQ).
...
-spec split_loop(list(ALX), integer(), list(ALX)) -> {list(ALX), list(ALX)}.
...
-spec split(list(ALT), integer()) -> {list(ALT), list(ALT)}.
split(List, Index) ->
    split_loop(List, Index, []).

-file("src/gleam/list.gleam", 1471).
-spec split_while_loop(list(AMG), fun((AMG) -> boolean()), list(AMG)) -> {list(AMG),
    list(AMG)}.
...
-spec split_while(list(AMC), fun((AMC) -> boolean())) -> {list(AMC), list(AMC)}.
...
-spec key_find(list({AML, AMM}), AML) -> {ok, AMM} | {error, nil}.
...
-spec key_filter(list({AMQ, AMR}), AMQ) -> list(AMR).
...
-spec key_pop_loop(list({ANA, ANB}), ANA, list({ANA, ANB})) -> {ok,
        {ANB, list({ANA, ANB})}} |
    {error, nil}.
...
-spec key_pop(list({AMU, AMV}), AMU) -> {ok, {AMV, list({AMU, AMV})}} |
    {error, nil}.
key_pop(List, Key) ->
    key_pop_loop(List, Key, []).

-file("src/gleam/list.gleam", 1606).
-spec key_set_loop(list({ANL, ANM}), ANL, ANM, list({ANL, ANM})) -> list({ANL,
    ANM}).
...
-spec key_set(list({ANH, ANI}), ANH, ANI) -> list({ANH, ANI}).
...
-spec each(list(ANQ), fun((ANQ) -> any())) -> nil.
each(List, F) ->
    case List of
        [] ->
            nil;
...
-spec try_each(list(ANT), fun((ANT) -> {ok, any()} | {error, ANW})) -> {ok, nil} |
    {error, ANW}.
try_each(List, Fun) ->
    case List of
        [] ->
...
-spec partition_loop(list(BGR), fun((BGR) -> boolean()), list(BGR), list(BGR)) -> {list(BGR),
    list(BGR)}.
...
-spec partition(list(AOB), fun((AOB) -> boolean())) -> {list(AOB), list(AOB)}.
partition(List, Categorise) ->
    partition_loop(List, Categorise, [], []).

-file("src/gleam/list.gleam", 1769).
-spec window_loop(list(list(API)), list(API), integer()) -> list(list(API)).
...
-spec window(list(APE), integer()) -> list(list(APE)).
window(List, N) ->
    case N =< 0 of
        true ->
            [];
...
-spec window_by_2(list(APO)) -> list({APO, APO}).
window_by_2(List) ->
...
-spec drop_while(list(APR), fun((APR) -> boolean())) -> list(APR).
...
-spec take_while_loop(list(APX), fun((APX) -> boolean()), list(APX)) -> list(APX).
...
-spec take_while(list(APU), fun((APU) -> boolean())) -> list(APU).
take_while(List, Predicate) ->
    take_while_loop(List, Predicate, []).

-file("src/gleam/list.gleam", 1863).
-spec chunk_loop(list(AQG), fun((AQG) -> AQI), AQI, list(AQG), list(list(AQG))) -> list(list(AQG)).
...
-spec sized_chunk(list(AQO), integer()) -> list(list(AQO)).
...
-spec reduce(list(AQZ), fun((AQZ, AQZ) -> AQZ)) -> {ok, AQZ} | {error, nil}.
reduce(List, Fun) ->
    case List of
        [] ->
            {error, nil};
...
-spec scan_loop(list(ARH), ARJ, list(ARJ), fun((ARJ, ARH) -> ARJ)) -> list(ARJ).
...
-spec scan(list(ARD), ARF, fun((ARF, ARD) -> ARF)) -> list(ARF).
...
-spec last(list(ARM)) -> {ok, ARM} | {error, nil}.
last(List) ->
    case List of
        [] ->
            {error, nil};
...
-spec combinations(list(ARQ), integer()) -> list(list(ARQ)).
...
-spec combination_pairs_loop(list(ARX), list({ARX, ARX})) -> list({ARX, ARX}).
...
-spec combination_pairs(list(ARU)) -> list({ARU, ARU}).
combination_pairs(Items) ->
    combination_pairs_loop(Items, []).

-file("src/gleam/list.gleam", 2107).
-spec take_firsts(list(list(ASR)), list(ASR), list(list(ASR))) -> {list(ASR),
    list(list(ASR))}.
...
-spec transpose_loop(list(list(ASK)), list(list(ASK))) -> list(list(ASK)).
...
-spec transpose(list(list(ASF))) -> list(list(ASF)).
...
-spec interleave(list(list(ASB))) -> list(ASB).
interleave(List) ->
    _pipe = List,
...
-spec shuffle_pair_unwrap_loop(list({float(), ATD}), list(ATD)) -> list(ATD).
...
-spec do_shuffle_by_pair_indexes(list({float(), ATH})) -> list({float(), ATH}).
...
-spec shuffle(list(ATA)) -> list(ATA).
shuffle(List) ->
    _pipe = List,
...
-spec max_loop(list(ATR), fun((ATR, ATR) -> gleam@order:order()), ATR) -> ATR.
...
-spec max(list(ATK), fun((ATK, ATK) -> gleam@order:order())) -> {ok, ATK} |
    {error, nil}.
max(List, Compare) ->
    case List of
        [] ->
...
-spec build_reservoir_loop(
    list(AUG),
    integer(),
    gleam@dict:dict(integer(), AUG)
) -> {gleam@dict:dict(integer(), AUG), list(AUG)}.
...
-spec build_reservoir(list(AUB), integer()) -> {gleam@dict:dict(integer(), AUB),
    list(AUB)}.
...
-spec sample(list(ATS), integer()) -> list(ATS).
sample(List, N) ->
...
-spec permutation_zip(list(AOO), list(AOO), list(list(AOO))) -> list(list(AOO)).
...
-spec permutations(list(AOK)) -> list(list(AOK)).
permutations(List) ->
    case List of
        [] ->
            [[]];
