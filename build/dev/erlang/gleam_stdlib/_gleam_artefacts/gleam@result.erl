...
-spec map({ok, CNE} | {error, CNF}, fun((CNE) -> CNI)) -> {ok, CNI} |
    {error, CNF}.
map(Result, Fun) ->
    case Result of
        {ok, X} ->
...
-spec map_error({ok, CNL} | {error, CNM}, fun((CNM) -> CNP)) -> {ok, CNL} |
    {error, CNP}.
...
-spec flatten({ok, {ok, CNS} | {error, CNT}} | {error, CNT}) -> {ok, CNS} |
    {error, CNT}.
flatten(Result) ->
    case Result of
        {ok, X} ->
...
-spec 'try'({ok, COA} | {error, COB}, fun((COA) -> {ok, COE} | {error, COB})) -> {ok,
        COE} |
    {error, COB}.
'try'(Result, Fun) ->
    case Result of
...
-spec unwrap({ok, COJ} | {error, any()}, COJ) -> COJ.
...
-spec lazy_unwrap({ok, CON} | {error, any()}, fun(() -> CON)) -> CON.
...
-spec unwrap_error({ok, any()} | {error, COS}, COS) -> COS.
...
-spec 'or'({ok, COV} | {error, COW}, {ok, COV} | {error, COW}) -> {ok, COV} |
    {error, COW}.
'or'(First, Second) ->
    case First of
        {ok, _} ->
...
-spec lazy_or({ok, CPD} | {error, CPE}, fun(() -> {ok, CPD} | {error, CPE})) -> {ok,
        CPD} |
    {error, CPE}.
...
-spec all(list({ok, CPL} | {error, CPM})) -> {ok, list(CPL)} | {error, CPM}.
all(Results) ->
    gleam@list:try_map(Results, fun(Result) -> Result end).

-file("src/gleam/result.gleam", 307).
-spec partition_loop(list({ok, CQA} | {error, CQB}), list(CQA), list(CQB)) -> {list(CQA),
    list(CQB)}.
...
-spec partition(list({ok, CPT} | {error, CPU})) -> {list(CPT), list(CPU)}.
partition(Results) ->
...
-spec replace({ok, any()} | {error, CQJ}, CQM) -> {ok, CQM} | {error, CQJ}.
...
-spec replace_error({ok, CQP} | {error, any()}, CQT) -> {ok, CQP} | {error, CQT}.
...
-spec values(list({ok, CQW} | {error, any()})) -> list(CQW).
values(Results) ->
