...
-spec map_first({CMN, CMO}, fun((CMN) -> CMP)) -> {CMP, CMO}.
map_first(Pair, Fun) ->
    {A, B} = Pair,
    {Fun(A), B}.
...
-spec map_second({CMQ, CMR}, fun((CMR) -> CMS)) -> {CMQ, CMS}.
map_second(Pair, Fun) ->
    {A, B} = Pair,
    {A, Fun(B)}.
