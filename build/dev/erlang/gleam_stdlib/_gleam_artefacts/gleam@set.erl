...
-opaque set(CVT) :: {set, gleam@dict:dict(CVT, list(nil))}.
...
-spec contains(set(CWE), CWE) -> boolean().
contains(Set, Member) ->
...
-spec delete(set(CWG), CWG) -> set(CWG).
delete(Set, Member) ->
...
-spec fold(set(CWP), CWR, fun((CWR, CWP) -> CWR)) -> CWR.
...
-spec filter(set(CWS), fun((CWS) -> boolean())) -> set(CWS).
...
-spec drop(set(CWZ), list(CWZ)) -> set(CWZ).
drop(Set, Disallowed) ->
...
-spec take(set(CXD), list(CXD)) -> set(CXD).
take(Set, Desired) ->
    {set, gleam@dict:take(erlang:element(2, Set), Desired)}.

-file("src/gleam/set.gleam", 290).
-spec order(set(CXL), set(CXL)) -> {set(CXL), set(CXL)}.
order(First, Second) ->
...
-spec intersection(set(CXQ), set(CXQ)) -> set(CXQ).
...
-spec difference(set(CXU), set(CXU)) -> set(CXU).
...
-spec is_subset(set(CXY), set(CXY)) -> boolean().
...
-spec is_disjoint(set(CYB), set(CYB)) -> boolean().
...
-spec each(set(CYI), fun((CYI) -> any())) -> nil.
each(Set, Fun) ->
    fold(
        Set,
        nil,
...
-spec insert(set(CWB), CWB) -> set(CWB).
insert(Set, Member) ->
...
-spec from_list(list(CWM)) -> set(CWM).
from_list(Members) ->
...
-spec map(set(CWV), fun((CWV) -> CWX)) -> set(CWX).
map(Set, Fun) ->
...
-spec union(set(CXH), set(CXH)) -> set(CXH).
union(First, Second) ->
    {Larger, Smaller} = order(First, Second),
...
-spec symmetric_difference(set(CYE), set(CYE)) -> set(CYE).
