...
-type dict(JU, JV) :: any() | {gleam_phantom, JU, JV}.

-type transient_dict(JW, JX) :: any() | {gleam_phantom, JW, JX}.
...
-spec from_list_loop(transient_dict(LC, LD), list({LC, LD})) -> dict(LC, LD).
...
-spec do_take_loop(dict(OR, OS), list(OR), transient_dict(OR, OS)) -> dict(OR, OS).
...
-spec take(dict(OD, OE), list(OD)) -> dict(OD, OE).
...
-spec delete(dict(PI, PJ), PI) -> dict(PI, PJ).
delete(Dict, Key) ->
...
-spec drop_loop(transient_dict(QI, QJ), list(QI)) -> dict(QI, QJ).
...
-spec drop(dict(PU, PV), list(PU)) -> dict(PU, PV).
...
-spec upsert(dict(QP, QQ), QP, fun((gleam@option:option(QQ)) -> QQ)) -> dict(QP, QQ).
upsert(Dict, Key, Fun) ->
    case gleam_stdlib:map_get(Dict, Key) of
        {ok, Value} ->
...
-spec fold(dict(QW, QX), RA, fun((RA, QW, QX) -> RA)) -> RA.
...
-spec values(dict(any(), NN)) -> list(NN).
values(Dict) ->
    maps:values(Dict).
...
-spec filter(dict(NR, NS), fun((NR, NS) -> boolean())) -> dict(NR, NS).
...
-spec each(dict(RG, RH), fun((RG, RH) -> any())) -> nil.
each(Dict, Fun) ->
    fold(
        Dict,
        nil,
...
-spec combine(dict(RL, RM), dict(RL, RM), fun((RM, RM) -> RM)) -> dict(RL, RM).
...
-spec merge(dict(PA, PB), dict(PA, PB)) -> dict(PA, PB).
merge(Dict, New_entries) ->
    maps:merge(Dict, New_entries).

-file("src/gleam/dict.gleam", 566).
-spec group_loop(transient_dict(SN, list(SO)), fun((SO) -> SN), list(SO)) -> dict(SN, list(SO)).
...
?DOC(false).
-spec group(fun((SH) -> SI), list(SH)) -> dict(SI, list(SH)).
group(Key, List) ->
