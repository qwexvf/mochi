...
-file("src/mochi/schema_printer.gleam", 236).
...
-file("src/mochi/schema_printer.gleam", 245).
...
-file("src/mochi/schema_printer.gleam", 276).
...
-file("src/mochi/schema_printer.gleam", 126).
...
-file("src/mochi/schema_printer.gleam", 86).
...
-file("src/mochi/schema_printer.gleam", 342).
?DOC(" Escape special characters in a string for SDL\n").
...
-file("src/mochi/schema_printer.gleam", 306).
?DOC(" Print a Dynamic value in GraphQL SDL format\n").
-spec print_value(gleam@dynamic:dynamic_()) -> binary().
print_value(Value) ->
...
-file("src/mochi/schema_printer.gleam", 194).
...
print_input_field_definition(Field, Indent) ->
    ...
    Base@1 = case erlang:element(5, Field) of
        {some, Value} ->
            <<<<Base/binary, " = "/utf8>>/binary, (print_value(Value))/binary>>;
...
-file("src/mochi/schema_printer.gleam", 145).
...
-file("src/mochi/schema_printer.gleam", 213).
...
-file("src/mochi/schema_printer.gleam", 165).
...
-file("src/mochi/schema_printer.gleam", 49).
...
-file("src/mochi/schema_printer.gleam", 106).
...
-file("src/mochi/schema_printer.gleam", 37).
...
-file("src/mochi/schema_printer.gleam", 16).
...
-file("src/mochi/schema_printer.gleam", 352).
