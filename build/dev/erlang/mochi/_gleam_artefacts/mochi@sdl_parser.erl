...
-file("src/mochi/sdl_parser.gleam", 392).
?DOC(
    " Convert a keyword token to its string representation\n"
    " GraphQL keywords are contextual and can be used as names\n"
).
-spec keyword_to_name(mochi@sdl_lexer:s_d_l_token()) -> {ok, binary()} |
    {error, nil}.
keyword_to_name(Token) ->
    case Token of
        type ->
            {ok, <<"type"/utf8>>};
...
-file("src/mochi/sdl_parser.gleam", 877).
...
-file("src/mochi/sdl_parser.gleam", 870).
...
-file("src/mochi/sdl_parser.gleam", 841).
...
-file("src/mochi/sdl_parser.gleam", 845).
...
-file("src/mochi/sdl_parser.gleam", 126).
?DOC(" Parse optional description (triple-quoted string)\n").
-spec parse_optional_description(s_d_l_parser()) -> {gleam@option:option(binary()),
    s_d_l_parser()}.
parse_optional_description(Parser) ->
    case peek_token(Parser) of
        {ok, {s_d_l_token_with_position, {description, Content}, _}} ->
            case consume_token(Parser) of
                {ok, {_, New_parser}} ->
                    {{some, Content}, New_parser};

                {error, _} ->
                    {none, Parser}
            end;

        {ok, {s_d_l_token_with_position, {string_value, Content@1}, _}} ->
            case consume_token(Parser) of
                {ok, {_, New_parser@1}} ->
                    {{some, Content@1}, New_parser@1};
...
-file("src/mochi/sdl_parser.gleam", 376).
...
parse_name(Parser) ->
    case consume_token(Parser) of
        ...
        {ok, {{s_d_l_token_with_position, Token, Position}, Parser@2}} ->
            case keyword_to_name(Token) of
                {ok, Name@1} ->
                    {ok, {Name@1, Parser@2}};

                {error, _} ->
                    {error,
                        {unexpected_token, <<"name"/utf8>>, Token, Position}}
            end;
...
-file("src/mochi/sdl_parser.gleam", 423).
...
-file("src/mochi/sdl_parser.gleam", 408).
...
-file("src/mochi/sdl_parser.gleam", 658).
...
-file("src/mochi/sdl_parser.gleam", 643).
...
-file("src/mochi/sdl_parser.gleam", 710).
...
-file("src/mochi/sdl_parser.gleam", 703).
...
-file("src/mochi/sdl_parser.gleam", 855).
...
-file("src/mochi/sdl_parser.gleam", 221).
-spec parse_directive_argument(s_d_l_parser()) -> {ok,
        {mochi@sdl_ast:directive_argument(), s_d_l_parser()}} |
    {error, s_d_l_parse_error()}.
parse_directive_argument(Parser) ->
    gleam@result:'try'(
        parse_name(Parser),
        fun(_use0) ->
            {Name, Parser@1} = _use0,
                ...
                fun(_use0@1) ->
                    ...
                    gleam@result:'try'(
                        ...
                        fun(_use0@2) ->
                            ...
                            {ok, {{directive_argument, Name, Value}, Parser@3}}
                        end
                    )
                end
            )
...
-file("src/mochi/sdl_parser.gleam", 204).
-spec parse_directive_arguments_loop(
    s_d_l_parser(),
    list(mochi@sdl_ast:directive_argument())
) -> {ok, {list(mochi@sdl_ast:directive_argument()), s_d_l_parser()}} |
    {error, s_d_l_parse_error()}.
parse_directive_arguments_loop(Parser, Acc) ->
    case peek_token(Parser) of
        ...
        {ok, {s_d_l_token_with_position, {name, _}, _}} ->
            gleam@result:'try'(
                ...
                fun(_use0) ->
                    ...
                    parse_directive_arguments_loop(Parser@1, [Argument | Acc])
                end
            );
...
-file("src/mochi/sdl_parser.gleam", 182).
?DOC(" Parse optional directive arguments: (arg: value, ...)\n").
-spec parse_optional_directive_arguments(s_d_l_parser()) -> {ok,
        {list(mochi@sdl_ast:directive_argument()), s_d_l_parser()}} |
    {error, s_d_l_parse_error()}.
        ...
        {ok, {s_d_l_token_with_position, left_paren, _}} ->
            gleam@result:'try'(
                ...
                fun(_use0) ->
                    {_, Parser@1} = _use0,
                    gleam@result:'try'(
                        parse_directive_arguments_loop(Parser@1, []),
                        fun(_use0@1) ->
...
-file("src/mochi/sdl_parser.gleam", 166).
?DOC(" Parse a single directive usage: @name or @name(arg: value, ...)\n").
-spec parse_directive_usage(s_d_l_parser()) -> {ok,
        {mochi@sdl_ast:directive_usage(), s_d_l_parser()}} |
    {error, s_d_l_parse_error()}.
parse_directive_usage(Parser) ->
    gleam@result:'try'(
        expect_token(Parser, at, <<"@"/utf8>>),
        fun(_use0) ->
            {_, Parser@1} = _use0,
            gleam@result:'try'(
                parse_name(Parser@1),
                fun(_use0@1) ->
                    ...
                    gleam@result:'try'(
                        parse_optional_directive_arguments(Parser@2),
                        fun(_use0@2) ->
                            {Arguments, Parser@3} = _use0@2,
                            {ok, {{directive_usage, Name, Arguments}, Parser@3}}
                        end
                    )
                end
            )
...
-file("src/mochi/sdl_parser.gleam", 152).
-spec parse_directives_loop(
    s_d_l_parser(),
    list(mochi@sdl_ast:directive_usage())
) -> {ok, {list(mochi@sdl_ast:directive_usage()), s_d_l_parser()}} |
    {error, s_d_l_parse_error()}.
parse_directives_loop(Parser, Acc) ->
    case peek_token(Parser) of
        {ok, {s_d_l_token_with_position, at, _}} ->
            gleam@result:'try'(
                parse_directive_usage(Parser),
                fun(_use0) ->
                    {Directive, Parser@1} = _use0,
                    parse_directives_loop(Parser@1, [Directive | Acc])
                end
            );
...
-file("src/mochi/sdl_parser.gleam", 146).
?DOC(" Parse optional directives (@name or @name(args...))\n").
-spec parse_optional_directives(s_d_l_parser()) -> {ok,
        {list(mochi@sdl_ast:directive_usage()), s_d_l_parser()}} |
    {error, s_d_l_parse_error()}.
parse_optional_directives(Parser) ->
    parse_directives_loop(Parser, []).

-file("src/mochi/sdl_parser.gleam", 288).
?DOC(" Parse: union SearchResult = User | Post\n").
-spec parse_union_type_definition(s_d_l_parser(), gleam@option:option(binary())) -> {ok,
        ...
        fun(_use0) ->
            ...
            gleam@result:'try'(
                ...
                fun(_use0@1) ->
                    {Name, Parser@2} = _use0@1,
                    gleam@result:'try'(
                        parse_optional_directives(Parser@2),
                        fun(_use0@2) ->
                            {Directives, Parser@3} = _use0@2,
                            gleam@result:'try'(
                                expect_token(Parser@3, equals, <<"="/utf8>>),
                                fun(_use0@3) ->
                                    {_, Parser@4} = _use0@3,
                                    gleam@result:'try'(
                                        parse_union_member_types(Parser@4),
                                        fun(_use0@4) ->
                                            {Members, Parser@5} = _use0@4,
                                            {ok,
                                                {{union_type_def,
                                                        Name,
                                                        Description,
...
-file("src/mochi/sdl_parser.gleam", 310).
?DOC(" Parse: scalar DateTime\n").
-spec parse_scalar_type_definition(
    s_d_l_parser(),
    gleam@option:option(binary())
) -> {ok, {mochi@sdl_ast:scalar_type_def(), s_d_l_parser()}} |
    {error, s_d_l_parse_error()}.
...
-file("src/mochi/sdl_parser.gleam", 507).
            ...
            gleam@result:'try'(
                ...
                fun(_use0@1) ->
                    ...
                    gleam@result:'try'(
                        parse_type(Parser@3),
                        fun(_use0@2) ->
                            {Arg_type, Parser@4} = _use0@2,
                            gleam@result:'try'(
                                parse_optional_default_value(Parser@4),
                                fun(_use0@3) ->
                                    {Default_value, Parser@5} = _use0@3,
                                    gleam@result:'try'(
                                        parse_optional_directives(Parser@5),
                                        fun(_use0@4) ->
                                            {Directives, Parser@6} = _use0@4,
                                            {ok,
                                                {{argument_def,
                                                        Name,
                                                        Description,
...
-file("src/mochi/sdl_parser.gleam", 585).
...
parse_argument_definitions(Parser, Acc) ->
    case peek_token(Parser) of
        ...
        {ok, {s_d_l_token_with_position, directive, _}} ->
            gleam@result:'try'(
                parse_argument_definition(Parser),
                fun(_use0) ->
                    {Argument, Parser@1} = _use0,
        ...
        {ok, {s_d_l_token_with_position, schema, _}} ->
            gleam@result:'try'(
                parse_argument_definition(Parser),
                fun(_use0) ->
                    {Argument, Parser@1} = _use0,
        ...
        {ok, {s_d_l_token_with_position, extend, _}} ->
            gleam@result:'try'(
                parse_argument_definition(Parser),
                fun(_use0) ->
                    {Argument, Parser@1} = _use0,
        ...
        {ok, {s_d_l_token_with_position, {description, _}, _}} ->
            gleam@result:'try'(
                parse_argument_definition(Parser),
                fun(_use0) ->
                    {Argument, Parser@1} = _use0,
        ...
        {ok, {s_d_l_token_with_position, {string_value, _}, _}} ->
            gleam@result:'try'(
                parse_argument_definition(Parser),
                fun(_use0) ->
                    {Argument, Parser@1} = _use0,
...
-file("src/mochi/sdl_parser.gleam", 563).
...
-file("src/mochi/sdl_parser.gleam", 481).
...
parse_field_definition(Parser) ->
    ...
    gleam@result:'try'(
        ...
        fun(_use0) ->
            {Name, Parser@2} = _use0,
            gleam@result:'try'(
                parse_optional_arguments_definition(Parser@2),
                fun(_use0@1) ->
                    {Arguments, Parser@3} = _use0@1,
                    gleam@result:'try'(
                        expect_token(Parser@3, colon, <<":"/utf8>>),
                        fun(_use0@2) ->
                            {_, Parser@4} = _use0@2,
                            gleam@result:'try'(
                                parse_type(Parser@4),
                                fun(_use0@3) ->
                                    {Field_type, Parser@5} = _use0@3,
                                    gleam@result:'try'(
                                        parse_optional_directives(Parser@5),
                                        fun(_use0@4) ->
                                            {Directives, Parser@6} = _use0@4,
...
-file("src/mochi/sdl_parser.gleam", 450).
...
-file("src/mochi/sdl_parser.gleam", 441).
...
-file("src/mochi/sdl_parser.gleam", 231).
?DOC(" Parse: type User { ... }\n").
-spec parse_object_type_definition(
    s_d_l_parser(),
    gleam@option:option(binary())
) -> {ok, {mochi@sdl_ast:object_type_def(), s_d_l_parser()}} |
    {error, s_d_l_parse_error()}.
                ...
                fun(_use0@1) ->
                    ...
                    gleam@result:'try'(
                        ...
                        fun(_use0@2) ->
                            ...
                            gleam@result:'try'(
                                parse_optional_directives(Parser@3),
                                fun(_use0@3) ->
                                    {Directives, Parser@4} = _use0@3,
                                    gleam@result:'try'(
                                        parse_fields_definition(Parser@4),
                                        fun(_use0@4) ->
                                            {Fields, Parser@5} = _use0@4,
                                            {ok,
                                                {{object_type_def,
                                                        Name,
                                                        Description,
...
-file("src/mochi/sdl_parser.gleam", 263).
?DOC(" Parse: interface Node { ... }\n").
-spec parse_interface_type_definition(
    s_d_l_parser(),
    gleam@option:option(binary())
) -> {ok, {mochi@sdl_ast:interface_type_def(), s_d_l_parser()}} |
    {error, s_d_l_parse_error()}.
...
-file("src/mochi/sdl_parser.gleam", 756).
    ...
    gleam@result:'try'(
        ...
        fun(_use0) ->
            ...
            gleam@result:'try'(
                parse_optional_directives(Parser@2),
                fun(_use0@1) ->
                    {Directives, Parser@3} = _use0@1,
                    {ok,
                        {{enum_value_def, Name, Description, Directives},
                            Parser@3}}
                end
            )
        end
...
-file("src/mochi/sdl_parser.gleam", 735).
...
-file("src/mochi/sdl_parser.gleam", 726).
...
-file("src/mochi/sdl_parser.gleam", 333).
?DOC(" Parse: enum Status { ACTIVE INACTIVE }\n").
-spec parse_enum_type_definition(s_d_l_parser(), gleam@option:option(binary())) -> {ok,
                ...
                fun(_use0@1) ->
                    ...
                    gleam@result:'try'(
                        ...
                        fun(_use0@2) ->
                            {Directives, Parser@3} = _use0@2,
                            gleam@result:'try'(
                                parse_enum_values_definition(Parser@3),
                                fun(_use0@3) ->
                                    {Values, Parser@4} = _use0@3,
                                    {ok,
                                        {{enum_type_def,
                                                Name,
                                                Description,
...
-file("src/mochi/sdl_parser.gleam", 813).
...
-file("src/mochi/sdl_parser.gleam", 784).
...
-file("src/mochi/sdl_parser.gleam", 775).
...
-file("src/mochi/sdl_parser.gleam", 354).
?DOC(" Parse: input CreateUserInput { ... }\n").
-spec parse_input_object_type_definition(
    s_d_l_parser(),
    gleam@option:option(binary())
) -> {ok, {mochi@sdl_ast:input_object_type_def(), s_d_l_parser()}} |
    {error, s_d_l_parse_error()}.
                ...
                fun(_use0@1) ->
                    ...
                    gleam@result:'try'(
                        ...
                        fun(_use0@2) ->
                            {Directives, Parser@3} = _use0@2,
                            gleam@result:'try'(
                                parse_input_fields_definition(Parser@3),
                                fun(_use0@3) ->
                                    {Fields, Parser@4} = _use0@3,
                                    {ok,
                                        {{input_object_type_def,
                                                Name,
                                                Description,
                                                Directives,
                                                Fields},
...
parse_type_system_definition(Parser) ->
    ...
    case peek_token(Parser@1) of
        {ok, {s_d_l_token_with_position, type, _}} ->
            _pipe = parse_object_type_definition(Parser@1, Description),
            gleam@result:map(
                _pipe,
                fun(Result) ->
                    {{type_definition,
        ...
        {ok, {s_d_l_token_with_position, interface, _}} ->
            _pipe@1 = parse_interface_type_definition(Parser@1, Description),
            gleam@result:map(
                _pipe@1,
                fun(Result@1) ->
                    {{type_definition,
        ...
        {ok, {s_d_l_token_with_position, union, _}} ->
            _pipe@2 = parse_union_type_definition(Parser@1, Description),
            gleam@result:map(
                _pipe@2,
                fun(Result@2) ->
                    {{type_definition,
        ...
        {ok, {s_d_l_token_with_position, scalar, _}} ->
            _pipe@3 = parse_scalar_type_definition(Parser@1, Description),
            gleam@result:map(
                _pipe@3,
                fun(Result@3) ->
                    {{type_definition,
        ...
        {ok, {s_d_l_token_with_position, enum, _}} ->
            _pipe@4 = parse_enum_type_definition(Parser@1, Description),
            gleam@result:map(
                _pipe@4,
                fun(Result@4) ->
                    {{type_definition,
        ...
        {ok, {s_d_l_token_with_position, input, _}} ->
            _pipe@5 = parse_input_object_type_definition(Parser@1, Description),
            gleam@result:map(
                _pipe@5,
                fun(Result@5) ->
                    {{type_definition,
