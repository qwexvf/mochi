...
fn route_request(req: Request, graphql_schema: mochi_schema.Schema) -> Response {
  case wisp.path_segments(req) {
    ...
    ["sandbox"] ->
      wisp.html_response(playground.apollo_sandbox("/graphql"), 200)
    ["explorer"] ->
    ...
    [] ->
      wisp.html_response(
        ...
        </style></head><body>" <> "<h1>🍡 Mochi GraphQL Server</h1>" <> "<p>Code-first GraphQL for Gleam, powered by the BEAM VM.</p>" <> "<h2>Interactive Playgrounds</h2>" <> "<div class=\"grid\">" <> "<div class=\"card\"><h3><a href=\"/graphiql\">GraphiQL</a></h3><p>The classic GraphQL IDE</p></div>" <> "<div class=\"card\"><h3><a href=\"/playground\">Playground</a></h3><p>GraphQL Playground (legacy)</p></div>" <> "<div class=\"card\"><h3><a href=\"/sandbox\">Apollo Sandbox</a></h3><p>Modern Apollo explorer</p></div>" <> "<div class=\"card\"><h3><a href=\"/explorer\">Simple Explorer</a></h3><p>Lightweight, no deps</p></div>" <> "</div>" <> "<h2>API Endpoints</h2>" <> "<ul>" <> "<li><code>POST /graphql</code> - Execute GraphQL queries</li>" <> "<li><code>GET /health</code> - Health check</li>" <> "<li><code>GET /metrics</code> - Performance metrics</li>" <> "<li><code>GET /cache/stats</code> - Cache statistics</li>" <> "</ul>" <> "<h2>Example Query</h2>" <> "<pre>curl -X POST http://localhost:8000/graphql \\\n" <> "  -H \"Content-Type: application/json\" \\\n" <> "  -d '{\"query\": \"{ users { id name email } }\"}'</pre>" <> "</body></html>",
        200,
      )
  ...
  let body =
    json.to_string(
      json.object([
        ...
        #(
          "hit_rate_percent",
          json.float(calculate_hit_rate(stats.hits, stats.misses)),
        ),
      ]),
    )
...
fn handle_graphql_route(
  req: Request,
  gql_schema: mochi_schema.Schema,
) -> Response {
  case req.method {
    ...
    _ -> {
      logging.log(
        logging.Warning,
        "Method not allowed: " <> http.method_to_string(req.method),
      )
