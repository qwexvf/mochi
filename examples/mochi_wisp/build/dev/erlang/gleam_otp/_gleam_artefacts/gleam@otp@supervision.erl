...
-type child_specification(PDC) :: {child_specification,
        fun(() -> {ok, gleam@otp@actor:started(PDC)} |
...
-spec worker(
    fun(() -> {ok, gleam@otp@actor:started(PDD)} |
        {error, gleam@otp@actor:start_error()})
) -> child_specification(PDD).
worker(Start) ->
...
-spec supervisor(
    fun(() -> {ok, gleam@otp@actor:started(PDI)} |
        {error, gleam@otp@actor:start_error()})
) -> child_specification(PDI).
supervisor(Start) ->
...
-spec significant(child_specification(PDN), boolean()) -> child_specification(PDN).
...
-spec timeout(child_specification(PDQ), integer()) -> child_specification(PDQ).
timeout(Child, Ms) ->
...
-spec restart(child_specification(PDT), restart()) -> child_specification(PDT).
...
-spec map_data(child_specification(PDW), fun((PDW) -> PDY)) -> child_specification(PDY).
