...
-type message(OJX) :: {message, OJX} |
    {system, gleam@otp@system:system_message()} |
    {unexpected, gleam@dynamic:dynamic_()}.

-opaque next(OJY, OJZ) :: {continue,
        OJY,
        gleam@option:option(gleam@erlang@process:selector(OJZ))} |
    {stop, gleam@erlang@process:exit_reason()}.

-type self(OKA, OKB) :: {self,
        gleam@otp@system:mode(),
        gleam@erlang@process:pid_(),
        OKA,
        gleam@erlang@process:selector(message(OKB)),
        gleam@otp@system:debug_state(),
        fun((OKA, OKB) -> next(OKA, OKB))}.

-type started(OKC) :: {started, gleam@erlang@process:pid_(), OKC}.

-opaque initialised(OKD, OKE, OKF) :: {initialised,
        OKD,
        gleam@option:option(gleam@erlang@process:selector(OKE)),
        OKF}.

-opaque builder(OKG, OKH, OKI) :: {builder,
        fun((gleam@erlang@process:subject(OKH)) -> {ok,
                initialised(OKG, OKH, OKI)} |
            {error, binary()}),
        integer(),
        fun((OKG, OKH) -> next(OKG, OKH)),
        gleam@option:option(gleam@erlang@process:name(OKH))}.

-type start_error() :: init_timeout |
    {init_failed, binary()} |
    {init_exited, gleam@erlang@process:exit_reason()}.

-type start_init_message(OKJ) :: {ack, {ok, OKJ} | {error, binary()}} |
...
-spec continue(OKO) -> next(OKO, any()).
continue(State) ->
    {continue, State, none}.
...
-spec with_selector(next(OLA, OLB), gleam@erlang@process:selector(OLB)) -> next(OLA, OLB).
...
-spec initialised(OLH) -> initialised(OLH, any(), nil).
initialised(State) ->
...
-spec selecting(
    initialised(OLM, any(), OLO),
    gleam@erlang@process:selector(OLS)
) -> initialised(OLM, OLS, OLO).
...
-spec returning(initialised(OLX, OLY, any()), OMD) -> initialised(OLX, OLY, OMD).
...
-spec new(OMH) -> builder(OMH, OMI, gleam@erlang@process:subject(OMI)).
new(State) ->
...
-spec new_with_initialiser(
    integer(),
    fun((gleam@erlang@process:subject(OMN)) -> {ok, initialised(OMP, OMN, OMQ)} |
        {error, binary()})
) -> builder(OMP, OMN, OMQ).
...
-spec on_message(builder(OMZ, ONA, ONB), fun((OMZ, ONA) -> next(OMZ, ONA))) -> builder(OMZ, ONA, ONB).
...
-spec named(builder(ONK, ONL, ONM), gleam@erlang@process:name(ONL)) -> builder(ONK, ONL, ONM).
named(Builder, Name) ->
    {builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec select_system_messages(gleam@erlang@process:selector(message(ONZ))) -> gleam@erlang@process:selector(message(ONZ)).
...
-spec receive_message(self(any(), ONV)) -> message(ONV).
...
-spec initialise_actor(
    builder(any(), any(), OOS),
    gleam@erlang@process:pid_(),
    gleam@erlang@process:subject({ok, OOS} | {error, binary()})
...
-spec start(builder(any(), any(), OPF)) -> {ok, started(OPF)} |
    {error, start_error()}.
start(Builder) ->
...
-spec send(gleam@erlang@process:subject(OPM), OPM) -> nil.
send(Subject, Msg) ->
...
-spec call(
    gleam@erlang@process:subject(OPO),
    integer(),
    fun((gleam@erlang@process:subject(OPQ)) -> OPO)
) -> OPQ.
