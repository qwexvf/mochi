...
-opaque supervisor(PFQ, PFR) :: {supervisor, gleam@erlang@process:pid_()} |
    {named_supervisor, gleam@erlang@process:name(message(PFQ, PFR))}.

-type message(PFS, PFT) :: any() | {gleam_phantom, PFS, PFT}.

-opaque builder(PFU, PFV) :: {builder,
        gleam@otp@supervision:child_type(),
        fun((PFU) -> {ok, gleam@otp@actor:started(PFV)} |
            {error, gleam@otp@actor:start_error()}),
        gleam@otp@supervision:restart(),
        integer(),
        integer(),
        gleam@option:option(gleam@erlang@process:name(message(PFU, PFV)))}.

-type erlang_start_flags() :: any().

-type erlang_supervisor_name(PFW, PFX) :: {local,
        gleam@erlang@process:name(message(PFW, PFX))}.

-type strategy() :: simple_one_for_one.

-type erlang_start_flag(PFY) :: {strategy, strategy()} |
    {intensity, integer()} |
    {period, integer()} |
    {gleam_phantom, PFY}.
...
-type erlang_child_spec_property(PFZ, PGA) :: {id, integer()} |
    {start,
        {gleam@erlang@atom:atom_(),
            gleam@erlang@atom:atom_(),
            list(fun((PFZ) -> {ok, gleam@otp@actor:started(PGA)} |
                {error, gleam@otp@actor:start_error()}))}} |
...
-spec get_by_name(gleam@erlang@process:name(message(PGB, PGC))) -> supervisor(PGB, PGC).
get_by_name(Name) ->
...
-spec named(builder(PGS, PGT), gleam@erlang@process:name(message(PGS, PGT))) -> builder(PGS, PGT).
named(Builder, Name) ->
    {builder,
...
-spec restart_tolerance(builder(PHB, PHC), integer(), integer()) -> builder(PHB, PHC).
...
-spec timeout(builder(PHH, PHI), integer()) -> builder(PHH, PHI).
timeout(Builder, Ms) ->
...
-spec restart_strategy(builder(PHN, PHO), gleam@otp@supervision:restart()) -> builder(PHN, PHO).
...
-spec start(builder(PHT, PHU)) -> {ok,
        gleam@otp@actor:started(supervisor(PHT, PHU))} |
    {error, gleam@otp@actor:start_error()}.
start(Builder) ->
...
-spec supervised(builder(PIS, PIT)) -> gleam@otp@supervision:child_specification(supervisor(PIS, PIT)).
supervised(Builder) ->
...
-spec start_child(supervisor(PIZ, PJA), PIZ) -> {ok,
        gleam@otp@actor:started(PJA)} |
    {error, gleam@otp@actor:start_error()}.
...
-spec start_child_callback(
    fun((PJX) -> {ok, gleam@otp@actor:started(PJY)} |
        {error, gleam@otp@actor:start_error()}),
    PJX
) -> gleam@otp@internal@result2:result2(gleam@erlang@process:pid_(), PJY, gleam@otp@actor:start_error()).
...
-spec worker_child(
    fun((PGI) -> {ok, gleam@otp@actor:started(PGJ)} |
        {error, gleam@otp@actor:start_error()})
) -> builder(PGI, PGJ).
...
-spec supervisor_child(
    fun((PGN) -> {ok, gleam@otp@actor:started(PGO)} |
        {error, gleam@otp@actor:start_error()})
) -> builder(PGN, PGO).
