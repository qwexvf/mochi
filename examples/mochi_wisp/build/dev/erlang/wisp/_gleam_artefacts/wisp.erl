...
-spec method_override(gleam@http@request:request(AIHT)) -> gleam@http@request:request(AIHT).
...
-spec sort_keys(list({binary(), AIJK})) -> list({binary(), AIJK}).
sort_keys(Pairs) ->
    gleam@list:sort(
        Pairs,
        fun(A, B) ->
...
-spec or_500({ok, AIIE} | {error, any()}) -> {ok, AIIE} |
    {error, gleam@http@response:response(body())}.
or_500(Result) ->
    case Result of
        {ok, Value} ->
...
-spec fn_with_bad_request_error(
    fun((AIIS) -> {ok, AIIT} | {error, any()}),
    binary()
) -> fun((AIIS) -> {ok, AIIT} | {error, gleam@http@response:response(body())}).
...
-spec multipart_body(
    ...
    integer(),
    integer(),
    fun((AIIM, bitstring()) -> {ok, AIIM} |
        {error, gleam@http@response:response(body())}),
    AIIM
) -> {ok, {gleam@option:option(buffered_reader()), integer(), AIIM}} |
    {error, gleam@http@response:response(body())}.
...
serve_static(Req, Prefix, Directory, Handler) ->
    ...
    case {erlang:element(2, Req),
        ...
        {get, true} ->
            ...
            Path@2 = case gleam_stdlib:percent_decode(Path@1) of
                {ok, P} ->
                    P;
            ...
            case simplifile_erl:file_info(Path@2) of
                {ok, File_info} ->
