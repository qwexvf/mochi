...
-spec set_header(request(NTF), binary(), binary()) -> request(NTF).
...
-spec prepend_header(request(NTI), binary(), binary()) -> request(NTI).
...
-spec set_body(request(any()), NTN) -> request(NTN).
set_body(Req, Body) ->
    {request,
...
-spec map(request(NTP), fun((NTP) -> NTR)) -> request(NTR).
...
-spec set_query(request(NUB), list({binary(), binary()})) -> request(NUB).
set_query(Req, Query) ->
    Query@1 = begin
...
-spec set_method(request(NUF), gleam@http:method()) -> request(NUF).
...
-spec set_scheme(request(NUM), gleam@http:scheme()) -> request(NUM).
...
-spec set_host(request(NUP), binary()) -> request(NUP).
set_host(Req, Host) ->
    {request,
        erlang:element(2, Req),
        erlang:element(3, Req),
...
-spec set_port(request(NUS), integer()) -> request(NUS).
set_port(Req, Port) ->
    {request,
        erlang:element(2, Req),
        erlang:element(3, Req),
...
-spec set_path(request(NUV), binary()) -> request(NUV).
set_path(Req, Path) ->
    {request,
        erlang:element(2, Req),
        erlang:element(3, Req),
...
-spec set_cookie(request(NUY), binary(), binary()) -> request(NUY).
...
-spec remove_cookie(request(NVE), binary()) -> request(NVE).
