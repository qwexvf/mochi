...
-type response(OCZ) :: {response, integer(), list({binary(), binary()}), OCZ}.
...
-spec set_header(response(ODO), binary(), binary()) -> response(ODO).
...
-spec prepend_header(response(ODR), binary(), binary()) -> response(ODR).
...
-spec set_body(response(any()), ODW) -> response(ODW).
...
-spec try_map(response(ODA), fun((ODA) -> {ok, ODC} | {error, ODD})) -> {ok,
        response(ODC)} |
    {error, ODD}.
...
-spec map(response(ODY), fun((ODY) -> OEA)) -> response(OEA).
...
-spec expire_cookie(response(OEJ), binary(), gleam@http@cookie:attributes()) -> response(OEJ).
