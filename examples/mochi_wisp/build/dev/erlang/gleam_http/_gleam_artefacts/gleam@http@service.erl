...
-spec map_response_body(
    fun((OGV) -> gleam@http@response:response(OGW)),
    fun((OGW) -> OGY)
) -> fun((OGV) -> gleam@http@response:response(OGY)).
...
-spec prepend_response_header(
    fun((OHA) -> gleam@http@response:response(OHB)),
    binary(),
    binary()
) -> fun((OHA) -> gleam@http@response:response(OHB)).
...
-spec ensure_post(gleam@http@request:request(OHE)) -> {ok,
        gleam@http@request:request(OHE)} |
    {error, nil}.
ensure_post(Req) ->
...
-spec method_override(fun((gleam@http@request:request(OHN)) -> OHP)) -> fun((gleam@http@request:request(OHN)) -> OHP).
