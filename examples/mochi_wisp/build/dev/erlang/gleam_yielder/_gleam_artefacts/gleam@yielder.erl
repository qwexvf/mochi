...
-type action(PUV) :: stop | {continue, PUV, fun(() -> action(PUV))}.

-opaque yielder(PUW) :: {yielder, fun(() -> action(PUW))}.

-type step(PUX, PUY) :: {next, PUX, PUY} | done.

-type chunk(PUZ, PVA) :: {another_by,
        list(PUZ),
        PVA,
        PUZ,
        fun(() -> action(PUZ))} |
...
-type sized_chunk(PVB) :: {another, list(PVB), fun(() -> action(PVB))} |
    {last, list(PVB)} |
    no_more.
...
-spec unfold_loop(PVJ, fun((PVJ) -> step(PVK, PVJ))) -> fun(() -> action(PVK)).
...
-spec unfold(PVE, fun((PVE) -> step(PVF, PVE))) -> yielder(PVF).
unfold(Initial, F) ->
    _pipe = Initial,
...
-spec repeatedly(fun(() -> PVO)) -> yielder(PVO).
repeatedly(F) ->
...
-spec from_list(list(PVS)) -> yielder(PVS).
from_list(List) ->
...
-spec transform(yielder(PWC), PWE, fun((PWE, PWC) -> step(PWF, PWE))) -> yielder(PWF).
transform(Yielder, Initial, F) ->
    _pipe = transform_loop(erlang:element(2, Yielder), Initial, F),
    {yielder, _pipe}.

-file("src/gleam/yielder.gleam", 204).
-spec fold_loop(fun(() -> action(PWM)), fun((PWO, PWM) -> PWO), PWO) -> PWO.
...
-spec fold(yielder(PWJ), PWL, fun((PWL, PWJ) -> PWL)) -> PWL.
...
-spec to_list(yielder(PWR)) -> list(PWR).
to_list(Yielder) ->
    _pipe = Yielder,
...
-spec step(yielder(PWU)) -> step(PWU, yielder(PWU)).
step(Yielder) ->
...
-spec take_loop(fun(() -> action(PXC)), integer()) -> fun(() -> action(PXC)).
...
-spec take(yielder(PWZ), integer()) -> yielder(PWZ).
...
-spec drop_loop(fun(() -> action(PXI)), integer()) -> action(PXI).
...
-spec map(yielder(PXL), fun((PXL) -> PXN)) -> yielder(PXN).
map(Yielder, F) ->
    _pipe = erlang:element(2, Yielder),
...
-spec map2(yielder(PXT), yielder(PXV), fun((PXT, PXV) -> PXX)) -> yielder(PXX).
...
-spec append_loop(fun(() -> action(PYJ)), fun(() -> action(PYJ))) -> action(PYJ).
...
-spec append(yielder(PYF), yielder(PYF)) -> yielder(PYF).
append(First, Second) ->
    _pipe = fun() ->
...
-spec flatten_loop(fun(() -> action(yielder(PYR)))) -> action(PYR).
...
-spec flatten(yielder(yielder(PYN))) -> yielder(PYN).
flatten(Yielder) ->
...
-spec concat(list(yielder(PYV))) -> yielder(PYV).
concat(Yielders) ->
...
-spec flat_map(yielder(PYZ), fun((PYZ) -> yielder(PZB))) -> yielder(PZB).
flat_map(Yielder, F) ->
    _pipe = Yielder,
    _pipe@1 = map(_pipe, F),
    flatten(_pipe@1).
...
-spec filter_loop(fun(() -> action(PZH)), fun((PZH) -> boolean())) -> action(PZH).
...
-spec filter(yielder(PZE), fun((PZE) -> boolean())) -> yielder(PZE).
...
-spec filter_map(yielder(PZK), fun((PZK) -> {ok, PZM} | {error, any()})) -> yielder(PZM).
...
-spec cycle(yielder(PZY)) -> yielder(PZY).
cycle(Yielder) ->
    _pipe = repeat(Yielder),
    flatten(_pipe).

-file("src/gleam/yielder.gleam", 709).
-spec find_loop(fun(() -> action(QAG)), fun((QAG) -> boolean())) -> {ok, QAG} |
    {error, nil}.
...
-spec find(yielder(QAC), fun((QAC) -> boolean())) -> {ok, QAC} | {error, nil}.
...
-spec find_map(yielder(QAK), fun((QAK) -> {ok, QAM} | {error, any()})) -> {ok,
        QAM} |
    {error, nil}.
...
-spec index_loop(fun(() -> action(QBD)), integer()) -> fun(() -> action({QBD,
    integer()})).
...
-spec index(yielder(QBA)) -> yielder({QBA, integer()}).
index(Yielder) ->
    _pipe = erlang:element(2, Yielder),
...
-spec iterate(QBG, fun((QBG) -> QBG)) -> yielder(QBG).
iterate(Initial, F) ->
    unfold(Initial, fun(Element) -> {next, Element, F(Element)} end).

-file("src/gleam/yielder.gleam", 832).
-spec take_while_loop(fun(() -> action(QBL)), fun((QBL) -> boolean())) -> fun(() -> action(QBL)).
...
-spec take_while(yielder(QBI), fun((QBI) -> boolean())) -> yielder(QBI).
...
-spec drop_while_loop(fun(() -> action(QBR)), fun((QBR) -> boolean())) -> action(QBR).
...
-spec drop_while(yielder(QBO), fun((QBO) -> boolean())) -> yielder(QBO).
...
-spec scan_loop(fun(() -> action(QBY)), fun((QCA, QBY) -> QCA), QCA) -> fun(() -> action(QCA)).
...
-spec scan(yielder(QBU), QBW, fun((QBW, QBU) -> QBW)) -> yielder(QBW).
...
-spec zip_loop(fun(() -> action(QCH)), fun(() -> action(QCJ))) -> fun(() -> action({QCH,
    QCJ})).
zip_loop(Left, Right) ->
    fun() -> case Left() of
            stop ->
...
-spec zip(yielder(QCC), yielder(QCE)) -> yielder({QCC, QCE}).
zip(Left, Right) ->
    _pipe = zip_loop(erlang:element(2, Left), erlang:element(2, Right)),
    {yielder, _pipe}.

-file("src/gleam/yielder.gleam", 1000).
-spec next_chunk(fun(() -> action(QCW)), fun((QCW) -> QCY), QCY, list(QCW)) -> chunk(QCW, QCY).
...
-spec chunk_loop(fun(() -> action(QCR)), fun((QCR) -> QCT), QCT, QCR) -> action(list(QCR)).
...
-spec chunk(yielder(QCM), fun((QCM) -> any())) -> yielder(list(QCM)).
chunk(Yielder, F) ->
...
-spec next_sized_chunk(fun(() -> action(QDK)), integer(), list(QDK)) -> sized_chunk(QDK).
...
-spec sized_chunk_loop(fun(() -> action(QDG)), integer()) -> fun(() -> action(list(QDG))).
...
-spec sized_chunk(yielder(QDC), integer()) -> yielder(list(QDC)).
...
-spec intersperse_loop(fun(() -> action(QDR)), QDR) -> action(QDR).
...
-spec intersperse(yielder(QDO), QDO) -> yielder(QDO).
...
-spec any_loop(fun(() -> action(QDW)), fun((QDW) -> boolean())) -> boolean().
...
-spec any(yielder(QDU), fun((QDU) -> boolean())) -> boolean().
any(Yielder, Predicate) ->
    _pipe = erlang:element(2, Yielder),
    any_loop(_pipe, Predicate).

-file("src/gleam/yielder.gleam", 1228).
-spec all_loop(fun(() -> action(QEA)), fun((QEA) -> boolean())) -> boolean().
...
-spec all(yielder(QDY), fun((QDY) -> boolean())) -> boolean().
all(Yielder, Predicate) ->
    _pipe = erlang:element(2, Yielder),
    all_loop(_pipe, Predicate).

-file("src/gleam/yielder.gleam", 1273).
-spec update_group_with(QEQ) -> fun((gleam@option:option(list(QEQ))) -> list(QEQ)).
...
-spec group_updater(fun((QEI) -> QEJ)) -> fun((gleam@dict:dict(QEJ, list(QEI)), QEI) -> gleam@dict:dict(QEJ, list(QEI))).
group_updater(F) ->
...
-spec group(yielder(QEC), fun((QEC) -> QEE)) -> gleam@dict:dict(QEE, list(QEC)).
group(Yielder, Key) ->
    _pipe = Yielder,
...
-spec reduce(yielder(QEU), fun((QEU, QEU) -> QEU)) -> {ok, QEU} | {error, nil}.
reduce(Yielder, F) ->
    case (erlang:element(2, Yielder))() of
        stop ->
            {error, nil};
...
-spec last(yielder(QEY)) -> {ok, QEY} | {error, nil}.
last(Yielder) ->
    _pipe = Yielder,
...
-spec single(QFG) -> yielder(QFG).
single(Elem) ->
    once(fun() -> Elem end).

-file("src/gleam/yielder.gleam", 1402).
-spec interleave_loop(fun(() -> action(QFM)), fun(() -> action(QFM))) -> action(QFM).
...
-spec interleave(yielder(QFI), yielder(QFI)) -> yielder(QFI).
...
-spec fold_until_loop(
    fun(() -> action(QFU)),
    fun((QFW, QFU) -> gleam@list:continue_or_stop(QFW)),
    QFW
) -> QFW.
...
-spec fold_until(
    yielder(QFQ),
    QFS,
    fun((QFS, QFQ) -> gleam@list:continue_or_stop(QFS))
) -> QFS.
...
-spec try_fold(yielder(QFY), QGA, fun((QGA, QFY) -> {ok, QGA} | {error, QGB})) -> {ok,
        QGA} |
    {error, QGB}.
...
-spec first(yielder(QGO)) -> {ok, QGO} | {error, nil}.
first(Yielder) ->
    case (erlang:element(2, Yielder))() of
        stop ->
            {error, nil};
...
-spec at(yielder(QGS), integer()) -> {ok, QGS} | {error, nil}.
at(Yielder, Index) ->
    _pipe = Yielder,
...
-spec each(yielder(QHA), fun((QHA) -> any())) -> nil.
each(Yielder, F) ->
    _pipe = Yielder,
    _pipe@1 = map(_pipe, F),
    run(_pipe@1).
...
-spec yield(QHD, fun(() -> yielder(QHD))) -> yielder(QHD).
yield(Element, Next) ->
    {yielder,
        fun() ->
...
-spec prepend(yielder(QHG), QHG) -> yielder(QHG).
