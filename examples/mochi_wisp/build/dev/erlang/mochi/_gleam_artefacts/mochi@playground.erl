-module(mochi@playground).
-compile([no_auto_import, nowarn_unused_vars, nowarn_unused_function, nowarn_nomatch, inline]).
-define(FILEPATH, "src/mochi/playground.gleam").
-export([graphiql/1, playground/1, apollo_sandbox/1, simple_explorer/1]).
...
-file("src/mochi/playground.gleam", 9).
?DOC(" Generate GraphiQL HTML page\n").
-spec graphiql(binary()) -> binary().
graphiql(Endpoint) ->
    <<<<"<!DOCTYPE html>
<html lang=\"en\">
<head>
...
-file("src/mochi/playground.gleam", 55).
?DOC(" Generate GraphQL Playground HTML page (legacy, but still popular)\n").
-spec playground(binary()) -> binary().
playground(Endpoint) ->
    <<<<"<!DOCTYPE html>
<html lang=\"en\">
<head>
...
-file("src/mochi/playground.gleam", 123).
?DOC(" Generate Apollo Sandbox HTML page (modern alternative)\n").
-spec apollo_sandbox(binary()) -> binary().
apollo_sandbox(Endpoint) ->
    <<<<"<!DOCTYPE html>
<html lang=\"en\">
<head>
  <meta charset=\"utf-8\" />
...
-file("src/mochi/playground.gleam", 157).
?DOC(" Generate a simple GraphQL explorer with no external dependencies\n").
-spec simple_explorer(binary()) -> binary().
simple_explorer(Endpoint) ->
    <<<<<<<<"<!DOCTYPE html>
<html lang=\"en\">
<head>
  <meta charset=\"utf-8\">
