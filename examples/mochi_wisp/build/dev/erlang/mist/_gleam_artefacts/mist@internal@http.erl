...
-file("src/mist/internal/http.gleam", 539).
?DOC(false).
-spec add_date_header(gleam@http@response:response(VFC)) -> gleam@http@response:response(VFC).
...
-file("src/mist/internal/http.gleam", 546).
?DOC(false).
-spec connection_close(gleam@http@response:response(VFF)) -> gleam@http@response:response(VFF).
...
?DOC(false).
-spec keep_alive(gleam@http@response:response(VFI)) -> gleam@http@response:response(VFI).
keep_alive(Resp) ->
    gleam@http@response:set_header(
        Resp,
        <<"connection"/utf8>>,
...
-file("src/mist/internal/http.gleam", 554).
?DOC(false).
-spec maybe_keep_alive(gleam@http@response:response(VFL)) -> gleam@http@response:response(VFL).
...
-file("src/mist/internal/http.gleam", 571).
?DOC(false).
-spec add_content_length(boolean(), integer()) -> fun((gleam@http@response:response(VFQ)) -> gleam@http@response:response(VFQ)).
...
-file("src/mist/internal/http.gleam", 591).
?DOC(false).
...
-file("src/mist/internal/http.gleam", 620).
?DOC(false).
                                        ...
                                        fun(_use0@1) ->
                                            ...
                                            gleam@result:'try'(
                                                ...
                                                fun(Host_header) ->
                                                    ...
                                                    Port@2 = case gleam_stdlib:parse_int(
                                                        Port
                                                    ) of
                                                        {ok, Port@1} ->
                                                            Port@1;
...
-file("src/mist/internal/http.gleam", 384).
?DOC(false).
...
-file("src/mist/internal/http.gleam", 469).
?DOC(false).
...
-file("src/mist/internal/http.gleam", 476).
?DOC(false).
...
-file("src/mist/internal/http.gleam", 516).
?DOC(false).
-spec upgrade(
    glisten@socket:socket(),
    glisten@transport:transport(),
