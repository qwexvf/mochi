...
-opaque next(XQM, XQN) :: {continue,
        XQM,
        gleam@option:option(gleam@erlang@process:selector(XQN))} |
    normal_stop |
...
-opaque builder(XQO, XQP) :: {builder,
        integer(),
        fun((gleam@http@request:request(XQO)) -> gleam@http@response:response(XQP)),
...
-type websocket_message(XQQ) :: {text, binary()} |
    {binary, bitstring()} |
    closed |
    shutdown |
    {custom, XQQ}.
...
-spec continue(XQR) -> next(XQR, any()).
continue(State) ->
    {continue, State, none}.

-file("src/mist.gleam", 58).
-spec with_selector(next(XQV, XQW), gleam@erlang@process:selector(XQW)) -> next(XQV, XQW).
...
-spec convert_next(next(XRK, XRL)) -> mist@internal@next:next(XRK, XRL).
convert_next(Next) ->
    case Next of
...
-spec new(
    fun((gleam@http@request:request(XSM)) -> gleam@http@response:response(XSO))
) -> builder(XSM, XSO).
new(Handler) ->
    {builder,
        4000,
...
-spec port(builder(XSS, XST), integer()) -> builder(XSS, XST).
port(Builder, Port) ->
    {builder,
        Port,
...
-spec read_request_body(
    builder(bitstring(), XSY),
    integer(),
    gleam@http@response:response(XSY)
) -> builder(mist@internal@http:connection(), XSY).
...
-spec bind(builder(XTK, XTL), binary()) -> builder(XTK, XTL).
...
-spec with_ipv6(builder(XTQ, XTR)) -> builder(XTQ, XTR).
with_ipv6(Builder) ->
    {builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec with_tls(builder(XTW, XTX), binary(), binary()) -> builder(XTW, XTX).
...
-spec internal_to_public_ws_message(
    mist@internal@websocket:handler_message(XUM)
) -> {ok, websocket_message(XUM)} | {error, nil}.
...
-spec websocket(
    gleam@http@request:request(mist@internal@http:connection()),
    fun((XUS, websocket_message(XUT), mist@internal@websocket:websocket_connection()) -> next(XUS, XUT)),
    fun((mist@internal@websocket:websocket_connection()) -> {XUS,
        gleam@option:option(gleam@erlang@process:selector(XUT))}),
    fun((XUS) -> nil)
...
-spec server_sent_events(
    ...
    fun((gleam@erlang@process:subject(XVH)) -> {ok,
            gleam@otp@actor:initialised(XVJ, XVH, any())} |
        {error, binary()}),
    fun((XVJ, XVH, s_s_e_connection()) -> gleam@otp@actor:next(XVJ, XVH))
) -> gleam@http@response:response(response_data()).
