...
-type valid_message(XCC) :: {socket_message, bitstring()} |
    socket_closed_message |
    {user_message, XCC}.

-type websocket_message(XCD) :: {valid, valid_message(XCD)} | invalid.
...
-type handler_message(XCE) :: {internal, gramps@websocket:frame()} | {user, XCE}.

-type websocket_state(XCF) :: {websocket_state,
        bitstring(),
        XCF,
...
?DOC(false).
-spec map_user_selector(gleam@option:option(gleam@erlang@process:selector(XDL))) -> gleam@option:option(gleam@erlang@process:selector(websocket_message(XDL))).
...
-spec apply_frames(
    ...
    fun((XDB, handler_message(XDC), websocket_connection()) -> mist@internal@next:next(XDB, XDC)),
    websocket_connection(),
    mist@internal@next:next(XDB, websocket_message(XDC)),
    fun((XDB) -> nil)
) -> mist@internal@next:next(XDB, websocket_message(XDC)).
...
-spec initialize_connection(
    fun((websocket_connection()) -> {XCO,
        gleam@option:option(gleam@erlang@process:selector(XCP))}),
    fun((XCO) -> nil),
    fun((XCO, handler_message(XCP), websocket_connection()) -> mist@internal@next:next(XCO, XCP)),
    glisten@socket:socket(),
    glisten@transport:transport(),
    list(binary())
) -> {ok,
        gleam@otp@actor:started(gleam@erlang@process:subject(websocket_message(XCP)))} |
    {error, nil}.
