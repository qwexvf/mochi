...
-spec push_key({ok, IOB} | {error, get_error()}, binary()) -> {ok, IOB} |
    {error, get_error()}.
push_key(Result, Key) ->
    case Result of
        {ok, T} ->
...
-spec expect_end_of_line(
    list(binary()),
    fun((list(binary())) -> {ok, {IQG, list(binary())}} | {error, parse_error()})
) -> {ok, {IQG, list(binary())}} | {error, parse_error()}.
...
drop_comments(Input, Acc, In_string) ->
    case Input of
        ...
        [<<"#"/utf8>> | Input@4] when In_string ->
            drop_comments(Input@4, [<<"#"/utf8>> | Acc], In_string);

        [<<"#"/utf8>> | Input@5] when not In_string ->
            _pipe = Input@5,
        ...
        [G@1 | Input@6] ->
            drop_comments(Input@6, [G@1 | Acc], In_string);

        [] ->
            lists:reverse(Acc)
    end.

-file("src/tom.gleam", 724).
-spec do(
    {ok, {IQR, list(binary())}} | {error, parse_error()},
    fun((IQR, list(binary())) -> {ok, IQU} | {error, parse_error()})
) -> {ok, IQU} | {error, parse_error()}.
do(Result, Next) ->
    case Result of
        {ok, {A, Input}} ->
            Next(A, Input);
...
-file("src/tom.gleam", 734).
-spec expect(
    list(binary()),
    binary(),
    fun((list(binary())) -> {ok, {IQZ, list(binary())}} | {error, parse_error()})
) -> {ok, {IQZ, list(binary())}} | {error, parse_error()}.
