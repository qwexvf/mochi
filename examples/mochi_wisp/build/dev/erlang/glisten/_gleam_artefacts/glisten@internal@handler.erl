...
-type message(RSS) :: {internal, internal_message()} | {user, RSS}.

-type loop_message(RST) :: {packet, bitstring()} | {custom, RST}.

-type loop_state(RSU, RSV) :: {loop_state,
        {ok, {glisten@socket@options:ip_address(), integer()}} | {error, nil},
        glisten@socket:socket(),
        gleam@erlang@process:subject(message(RSV)),
        glisten@transport:transport(),
        RSU}.

-type connection(RSW) :: {connection,
        {ok, {glisten@socket@options:ip_address(), integer()}} | {error, nil},
        glisten@socket:socket(),
        glisten@transport:transport(),
        gleam@erlang@process:subject(message(RSW))}.

-type next(RSX, RSY) :: {continue,
        RSX,
        gleam@option:option(gleam@erlang@process:selector(RSY))} |
    normal_stop |
    {abnormal_stop, binary()}.

-type handler(RSZ, RTA) :: {handler,
        glisten@socket:socket(),
        fun((RSZ, loop_message(RTA), connection(RTA)) -> next(RSZ, loop_message(RTA))),
        fun((connection(RTA)) -> {RSZ,
            gleam@option:option(gleam@erlang@process:selector(RTA))}),
        gleam@option:option(fun((RSZ) -> nil)),
        glisten@transport:transport()}.

-file("src/glisten/internal/handler.gleam", 65).
?DOC(false).
-spec continue(RTN) -> next(RTN, any()).
continue(State) ->
    {continue, State, none}.

-file("src/glisten/internal/handler.gleam", 69).
?DOC(false).
-spec with_selector(next(RTR, RTS), gleam@erlang@process:selector(RTS)) -> next(RTR, RTS).
...
?DOC(false).
-spec start(handler(any(), RUH)) -> {ok,
        gleam@otp@actor:started(gleam@erlang@process:subject(message(RUH)))} |
