...
-type message(SMT) :: {packet, bitstring()} | {user, SMT}.
...
-type connection(SMU) :: {connection,
        glisten@socket:socket(),
        glisten@transport:transport(),
        gleam@erlang@process:subject(glisten@internal@handler:message(SMU))}.

-opaque next(SMV, SMW) :: {continue,
        SMV,
        gleam@option:option(gleam@erlang@process:selector(SMW))} |
    normal_stop |
    {abnormal_stop, binary()}.

-opaque builder(SMX, SMY) :: {builder,
        glisten@socket@options:interface(),
        fun((connection(SMY)) -> {SMX,
            gleam@option:option(gleam@erlang@process:selector(SMY))}),
        fun((SMX, message(SMY), connection(SMY)) -> next(SMX, message(SMY))),
        gleam@option:option(fun((SMX) -> nil)),
        integer(),
        boolean(),
        boolean(),
...
-spec continue(SNY) -> next(SNY, any()).
continue(State) ->
    {continue, State, none}.

-file("src/glisten.gleam", 151).
-spec with_selector(next(SOC, SOD), gleam@erlang@process:selector(SOD)) -> next(SOC, SOD).
...
?DOC(false).
-spec convert_next(next(SOR, SOS)) -> glisten@internal@handler:next(SOR, SOS).
convert_next(Next) ->
    case Next of
...
?DOC(false).
-spec map_selector(next(SOX, SOY), fun((SOY) -> SPB)) -> next(SOX, SPB).
...
-spec map_user_selector(gleam@erlang@process:selector(message(SPE))) -> gleam@erlang@process:selector(glisten@internal@handler:loop_message(SPE)).
...
-spec convert_loop(
    fun((SPJ, message(SPK), connection(SPK)) -> next(SPJ, message(SPK)))
) -> fun((SPJ, glisten@internal@handler:loop_message(SPK), glisten@internal@handler:connection(SPK)) -> glisten@internal@handler:next(SPJ, glisten@internal@handler:loop_message(SPK))).
convert_loop(Loop) ->
    fun(Data, Msg, Conn) ->
        Conn@1 = {connection,
            erlang:element(3, Conn),
...
-spec convert_on_init(
    fun((connection(SPP)) -> {SPR,
        gleam@option:option(gleam@erlang@process:selector(SPP))})
) -> fun((glisten@internal@handler:connection(SPP)) -> {SPR,
    gleam@option:option(gleam@erlang@process:selector(SPP))}).
...
-spec new(
    fun((connection(SPX)) -> {SPZ,
        gleam@option:option(gleam@erlang@process:selector(SPX))}),
    fun((SPZ, message(SPX), connection(SPX)) -> next(SPZ, message(SPX)))
) -> builder(SPZ, SPX).
new(On_init, Loop) ->
...
-spec with_close(builder(SQG, SQH), fun((SQG) -> nil)) -> builder(SQG, SQH).
...
-spec with_pool_size(builder(SQM, SQN), integer()) -> builder(SQM, SQN).
...
?DOC(false).
-spec with_http2(builder(SQS, SQT)) -> builder(SQS, SQT).
with_http2(Builder) ->
    {builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec with_ipv6(builder(SRE, SRF)) -> builder(SRE, SRF).
with_ipv6(Builder) ->
    {builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec with_tls(builder(SRK, SRL), binary(), binary()) -> builder(SRK, SRL).
...
-spec bind(builder(SQY, SQZ), binary()) -> builder(SQY, SQZ).
