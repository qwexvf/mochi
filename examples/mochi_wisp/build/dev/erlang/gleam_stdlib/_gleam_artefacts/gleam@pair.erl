...
-spec map_first({CMS, CMT}, fun((CMS) -> CMU)) -> {CMU, CMT}.
map_first(Pair, Fun) ->
    {A, B} = Pair,
    {Fun(A), B}.
...
-spec map_second({CMV, CMW}, fun((CMW) -> CMX)) -> {CMV, CMX}.
map_second(Pair, Fun) ->
    {A, B} = Pair,
    {A, Fun(B)}.
