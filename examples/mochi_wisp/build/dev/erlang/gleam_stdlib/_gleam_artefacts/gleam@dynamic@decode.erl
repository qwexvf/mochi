...
-opaque decoder(BWF) :: {decoder,
        fun((gleam@dynamic:dynamic_()) -> {BWF, list(decode_error())})}.
...
-spec run(gleam@dynamic:dynamic_(), decoder(BWN)) -> {ok, BWN} |
...
-file("src/gleam/dynamic/decode.gleam", 889).
?DOC(
...
-spec map(decoder(CAL), fun((CAL) -> CAN)) -> decoder(CAN).
...
-file("src/gleam/dynamic/decode.gleam", 898).
...
-file("src/gleam/dynamic/decode.gleam", 936).
?DOC(
...
-spec then(decoder(CAX), fun((CAX) -> decoder(CAZ))) -> decoder(CAZ).
then(Decoder, Next) ->
    {decoder,
        fun(Dynamic_data) ->
...
-file("src/gleam/dynamic/decode.gleam", 979).
-spec run_decoders(
    gleam@dynamic:dynamic_(),
    {CBH, list(decode_error())},
    list(decoder(CBH))
) -> {CBH, list(decode_error())}.
...
-file("src/gleam/dynamic/decode.gleam", 966).
?DOC(
...
-spec one_of(decoder(CBC), list(decoder(CBC))) -> decoder(CBC).
...
-file("src/gleam/dynamic/decode.gleam", 1090).
?DOC(
...
-spec recursive(fun(() -> decoder(CBS))) -> decoder(CBS).
recursive(Inner) ->
    {decoder,
        fun(Data) ->
            Decoder = Inner(),
...
-file("src/gleam/dynamic/decode.gleam", 867).
?DOC(
...
-spec optional(decoder(CAH)) -> decoder(gleam@option:option(CAH)).
optional(Inner) ->
...
-spec run_dynamic_function(
    gleam@dynamic:dynamic_(),
    binary(),
    fun((gleam@dynamic:dynamic_()) -> {ok, BYI} | {error, BYI})
...
-file("src/gleam/dynamic/decode.gleam", 687).
...
-file("src/gleam/dynamic/decode.gleam", 713).
...
-file("src/gleam/dynamic/decode.gleam", 747).
...
-spec collapse_errors(decoder(CAU), binary()) -> decoder(CAU).
...
-file("src/gleam/dynamic/decode.gleam", 1012).
?DOC(
...
-spec failure(CBM, binary()) -> decoder(CBM).
failure(Placeholder, Name) ->
    {decoder, fun(D) -> {Placeholder, decode_error(Name, D)} end}.

-file("src/gleam/dynamic/decode.gleam", 1053).
?DOC(
...
-spec new_primitive_decoder(
    binary(),
    fun((gleam@dynamic:dynamic_()) -> {ok, CBO} | {error, CBO})
) -> decoder(CBO).
...
-file("src/gleam/dynamic/decode.gleam", 821).
-spec fold_dict(
    {gleam@dict:dict(BZT, BZU), list(decode_error())},
    gleam@dynamic:dynamic_(),
    gleam@dynamic:dynamic_(),
    fun((gleam@dynamic:dynamic_()) -> {BZT, list(decode_error())}),
    fun((gleam@dynamic:dynamic_()) -> {BZU, list(decode_error())})
) -> {gleam@dict:dict(BZT, BZU), list(decode_error())}.
...
-file("src/gleam/dynamic/decode.gleam", 801).
?DOC(
...
-spec dict(decoder(BZM), decoder(BZO)) -> decoder(gleam@dict:dict(BZM, BZO)).
dict(Key, Value) ->
...
-file("src/gleam/dynamic/decode.gleam", 769).
?DOC(
...
-spec list(decoder(BZA)) -> decoder(list(BZA)).
list(Inner) ->
    {decoder,
        fun(Data) ->
            gleam_stdlib:list(
...
-spec push_path({BXJ, list(decode_error())}, list(any())) -> {BXJ,
    list(decode_error())}.
...
-spec index(
    list(BWX),
    list(BWX),
    fun((gleam@dynamic:dynamic_()) -> {BXA, list(decode_error())}),
    gleam@dynamic:dynamic_(),
    fun((gleam@dynamic:dynamic_(), list(BWX)) -> {BXA, list(decode_error())})
) -> {BXA, list(decode_error())}.
...
-spec subfield(list(any()), decoder(BWI), fun((BWI) -> decoder(BWK))) -> decoder(BWK).
...
-spec at(list(any()), decoder(BWU)) -> decoder(BWU).
at(Path, Inner) ->
    {decoder,
        fun(Data) ->
            index(
...
-spec field(any(), decoder(BXS), fun((BXS) -> decoder(BXU))) -> decoder(BXU).
...
-spec optional_field(any(), BXY, decoder(BXY), fun((BXY) -> decoder(BYA))) -> decoder(BYA).
...
-spec optionally_at(list(any()), BYF, decoder(BYF)) -> decoder(BYF).
