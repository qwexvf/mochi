...
-type option(EV) :: {some, EV} | none.

-file("src/gleam/option.gleam", 59).
-spec reverse_and_prepend(list(FK), list(FK)) -> list(FK).
...
-spec all_loop(list(option(FB)), list(FB)) -> option(list(FB)).
all_loop(List, Acc) ->
    case List of
        [] ->
...
-spec all(list(option(EW))) -> option(list(EW)).
all(List) ->
    all_loop(List, []).
...
-spec to_result(option(FS), FV) -> {ok, FS} | {error, FV}.
to_result(Option, E) ->
    case Option of
        {some, A} ->
            {ok, A};
...
-spec from_result({ok, FY} | {error, any()}) -> option(FY).
from_result(Result) ->
    case Result of
        {ok, A} ->
            {some, A};
...
-spec lazy_unwrap(option(GF), fun(() -> GF)) -> GF.
...
-spec map(option(GH), fun((GH) -> GJ)) -> option(GJ).
map(Option, Fun) ->
    case Option of
        {some, X} ->
            {some, Fun(X)};
...
-spec flatten(option(option(GL))) -> option(GL).
flatten(Option) ->
    case Option of
        {some, X} ->
            X;
...
-spec then(option(GP), fun((GP) -> option(GR))) -> option(GR).
then(Option, Fun) ->
    case Option of
        {some, X} ->
            Fun(X);
...
-spec 'or'(option(GU), option(GU)) -> option(GU).
'or'(First, Second) ->
    case First of
        {some, _} ->
            First;
...
-spec lazy_or(option(GY), fun(() -> option(GY))) -> option(GY).
...
-spec values_loop(list(option(HG)), list(HG)) -> list(HG).
...
-spec values(list(option(HC))) -> list(HC).
values(Options) ->
    values_loop(Options, []).
