...
-spec map({ok, CNJ} | {error, CNK}, fun((CNJ) -> CNN)) -> {ok, CNN} |
    {error, CNK}.
map(Result, Fun) ->
    case Result of
        {ok, X} ->
...
-spec map_error({ok, CNQ} | {error, CNR}, fun((CNR) -> CNU)) -> {ok, CNQ} |
    {error, CNU}.
...
-spec flatten({ok, {ok, CNX} | {error, CNY}} | {error, CNY}) -> {ok, CNX} |
    {error, CNY}.
flatten(Result) ->
    case Result of
        {ok, X} ->
...
-spec 'try'({ok, COF} | {error, COG}, fun((COF) -> {ok, COJ} | {error, COG})) -> {ok,
        COJ} |
    {error, COG}.
'try'(Result, Fun) ->
    case Result of
...
-file("src/gleam/result.gleam", 183).
?DOC(
...
-spec unwrap({ok, COO} | {error, any()}, COO) -> COO.
...
-file("src/gleam/result.gleam", 205).
?DOC(
...
-spec lazy_unwrap({ok, COS} | {error, any()}, fun(() -> COS)) -> COS.
...
-file("src/gleam/result.gleam", 227).
?DOC(
...
-spec unwrap_error({ok, any()} | {error, COX}, COX) -> COX.
...
-spec 'or'({ok, CPA} | {error, CPB}, {ok, CPA} | {error, CPB}) -> {ok, CPA} |
    {error, CPB}.
'or'(First, Second) ->
    case First of
        {ok, _} ->
...
-file("src/gleam/result.gleam", 291).
?DOC(
...
-spec lazy_or({ok, CPI} | {error, CPJ}, fun(() -> {ok, CPI} | {error, CPJ})) -> {ok,
        CPI} |
    {error, CPJ}.
...
-file("src/gleam/result.gleam", 317).
?DOC(
...
-spec all(list({ok, CPQ} | {error, CPR})) -> {ok, list(CPQ)} | {error, CPR}.
all(Results) ->
    gleam@list:try_map(Results, fun(Result) -> Result end).

-file("src/gleam/result.gleam", 337).
-spec partition_loop(list({ok, CQF} | {error, CQG}), list(CQF), list(CQG)) -> {list(CQF),
    list(CQG)}.
...
-spec partition(list({ok, CPY} | {error, CPZ})) -> {list(CPY), list(CPZ)}.
partition(Results) ->
    partition_loop(Results, [], []).

-file("src/gleam/result.gleam", 359).
?DOC(
...
-spec replace({ok, any()} | {error, CQO}, CQR) -> {ok, CQR} | {error, CQO}.
...
-spec replace_error({ok, CQU} | {error, any()}, CQY) -> {ok, CQU} | {error, CQY}.
...
-spec values(list({ok, CRB} | {error, any()})) -> list(CRB).
values(Results) ->
    gleam@list:filter_map(Results, fun(Result) -> Result end).

-file("src/gleam/result.gleam", 429).
?DOC(
    " Updates a value held within the `Error` of a result by calling a given function\n"
