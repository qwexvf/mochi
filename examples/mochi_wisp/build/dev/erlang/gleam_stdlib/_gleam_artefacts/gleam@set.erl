...
-opaque set(CVY) :: {set, gleam@dict:dict(CVY, list(nil))}.
...
-spec contains(set(CWJ), CWJ) -> boolean().
contains(Set, Member) ->
...
-spec delete(set(CWL), CWL) -> set(CWL).
delete(Set, Member) ->
...
-spec fold(set(CWU), CWW, fun((CWW, CWU) -> CWW)) -> CWW.
...
-spec filter(set(CWX), fun((CWX) -> boolean())) -> set(CWX).
filter(Set, Predicate) ->
    {set,
        gleam@dict:filter(erlang:element(2, Set), fun(M, _) -> Predicate(M) end)}.

-file("src/gleam/set.gleam", 251).
?DOC(
...
-spec drop(set(CXE), list(CXE)) -> set(CXE).
drop(Set, Disallowed) ->
    gleam@list:fold(Disallowed, Set, fun delete/2).

-file("src/gleam/set.gleam", 269).
?DOC(
...
-spec take(set(CXI), list(CXI)) -> set(CXI).
take(Set, Desired) ->
    {set, gleam@dict:take(erlang:element(2, Set), Desired)}.

-file("src/gleam/set.gleam", 289).
-spec order(set(CXQ), set(CXQ)) -> {set(CXQ), set(CXQ)}.
order(First, Second) ->
...
-spec intersection(set(CXV), set(CXV)) -> set(CXV).
...
-spec difference(set(CXZ), set(CXZ)) -> set(CXZ).
difference(First, Second) ->
    drop(First, to_list(Second)).

-file("src/gleam/set.gleam", 346).
?DOC(
...
-spec is_subset(set(CYD), set(CYD)) -> boolean().
is_subset(First, Second) ->
    intersection(First, Second) =:= First.

-file("src/gleam/set.gleam", 364).
?DOC(
...
-spec is_disjoint(set(CYG), set(CYG)) -> boolean().
is_disjoint(First, Second) ->
    intersection(First, Second) =:= new().

-file("src/gleam/set.gleam", 408).
?DOC(
...
-spec each(set(CYN), fun((CYN) -> any())) -> nil.
each(Set, Fun) ->
    fold(
        Set,
        nil,
...
-spec insert(set(CWG), CWG) -> set(CWG).
insert(Set, Member) ->
...
-spec from_list(list(CWR)) -> set(CWR).
from_list(Members) ->
...
-spec map(set(CXA), fun((CXA) -> CXC)) -> set(CXC).
map(Set, Fun) ->
    fold(Set, new(), fun(Acc, Member) -> insert(Acc, Fun(Member)) end).

-file("src/gleam/set.gleam", 284).
?DOC(
...
-spec union(set(CXM), set(CXM)) -> set(CXM).
union(First, Second) ->
    {Larger, Smaller} = order(First, Second),
    fold(Smaller, Larger, fun insert/2).

-file("src/gleam/set.gleam", 378).
?DOC(
...
-spec symmetric_difference(set(CYJ), set(CYJ)) -> set(CYJ).
