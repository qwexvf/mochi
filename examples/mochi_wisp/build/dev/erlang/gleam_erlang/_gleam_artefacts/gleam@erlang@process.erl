...
-opaque subject(MXD) :: {subject, pid_(), gleam@dynamic:dynamic_()} |
    {named_subject, name(MXD)}.

-type name(MXE) :: any() | {gleam_phantom, MXE}.

-type do_not_leak() :: any().

-type selector(MXF) :: any() | {gleam_phantom, MXF}.
...
-spec 'receive'(subject(MYD), integer()) -> {ok, MYD} | {error, nil}.
...
-spec receive_forever(subject(MYL)) -> MYL.
...
-spec selector_receive(selector(MYP), integer()) -> {ok, MYP} | {error, nil}.
...
-spec selector_receive_forever(selector(MYT)) -> MYT.
...
-spec map_selector(selector(MYV), fun((MYV) -> MYX)) -> selector(MYX).
map_selector(A, B) ->
...
-spec merge_selector(selector(MYZ), selector(MYZ)) -> selector(MYZ).
merge_selector(A, B) ->
...
-spec select_map(selector(MZK), subject(MZM), fun((MZM) -> MZK)) -> selector(MZK).
...
-spec select(selector(MZG), subject(MZG)) -> selector(MZG).
...
-spec select_record(
    ...
    any(),
    integer(),
    fun((gleam@dynamic:dynamic_()) -> MZU)
) -> selector(MZU).
...
-spec select_other(selector(MZY), fun((gleam@dynamic:dynamic_()) -> MZY)) -> selector(MZY).
...
-spec deselect(selector(MZP), subject(any())) -> selector(MZP).
...
-spec select_specific_monitor(selector(NAK), monitor(), fun((down()) -> NAK)) -> selector(NAK).
...
-spec select_monitors(selector(NAN), fun((down()) -> NAN)) -> selector(NAN).
...
-spec select_trapped_exits(selector(MZD), fun((exit_message()) -> MZD)) -> selector(MZD).
...
-spec deselect_specific_monitor(selector(NAQ), monitor()) -> selector(NAQ).
...
-spec send_after(subject(NBM), integer(), NBM) -> timer().
...
-spec perform_call(
    subject(NAT),
    fun((subject(NAV)) -> NAT),
    fun((selector(NAV)) -> {ok, NAV} | {error, nil})
) -> NAV.
...
-spec call(subject(NBA), integer(), fun((subject(NBC)) -> NBA)) -> NBC.
...
-spec call_forever(subject(NBE), fun((subject(NBG)) -> NBE)) -> NBG.
