...
-opaque trie(DLH, DLI) :: {trie,
        gleam@option:option(DLI),
        gleam@dict:dict(DLH, trie(DLH, DLI))}.
...
-spec do_delete(trie(DLQ, DLR), list(DLQ)) -> gleam@option:option(trie(DLQ, DLR)).
do_delete(Trie, Path) ->
    case {Path, Trie} of
        {[], {trie, _, Children_map}} ->
...
-spec fold(trie(DLY, DLZ), DMC, fun((DMC, list(DLY), DLZ) -> DMC)) -> DMC.
...
-spec get(trie(DMK, DML), list(DMK)) -> {ok, DML} | {error, nil}.
get(From, Path) ->
    case {Path, From} of
        {[], {trie, none, _}} ->
            {error, nil};
...
-spec has_path(trie(DMR, any()), list(DMR)) -> boolean().
has_path(Trie, Path) ->
...
-spec map(trie(DNH, DNI), fun((DNI) -> DNL)) -> trie(DNH, DNL).
map(Trie, Fun) ->
    {trie,
...
-spec delete(trie(DLJ, DLK), list(DLJ)) -> trie(DLJ, DLK).
delete(Trie, Path) ->
...
-spec insert(trie(DMW, DMX), list(DMW), DMX) -> trie(DMW, DMX).
...
-spec from_list(list({list(DME), DMG})) -> trie(DME, DMG).
from_list(List) ->
    gleam@list:fold(
        List,
        new(),
...
-spec paths(trie(DNS, any())) -> list(list(DNS)).
paths(Trie) ->
...
-spec singleton(list(DNY), DOA) -> trie(DNY, DOA).
...
-spec subtrie(trie(DOH, DOI), list(DOH)) -> {ok, trie(DOH, DOI)} | {error, nil}.
subtrie(Trie, Prefix) ->
    case {Prefix, Trie} of
        {[], _} ->
            {ok, Trie};
...
-spec to_list(trie(DOQ, DOR)) -> list({list(DOQ), DOR}).
to_list(Trie) ->
...
-spec do_update(
    trie(DPF, DPG),
    list(DPF),
    fun((gleam@option:option(DPG)) -> gleam@option:option(DPG))
) -> gleam@option:option(trie(DPF, DPG)).
...
-spec update(
    trie(DOW, DOX),
    list(DOW),
    fun((gleam@option:option(DOX)) -> gleam@option:option(DOX))
) -> trie(DOW, DOX).
...
-spec values(trie(any(), DPQ)) -> list(DPQ).
values(Trie) ->
