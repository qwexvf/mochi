...
-spec parse_bits(bitstring(), gleam@dynamic@decode:decoder(EPR)) -> {ok, EPR} |
    {error, decode_error()}.
...
-spec do_parse(binary(), gleam@dynamic@decode:decoder(EPL)) -> {ok, EPL} |
    {error, decode_error()}.
...
-spec parse(binary(), gleam@dynamic@decode:decoder(EPH)) -> {ok, EPH} |
    {error, decode_error()}.
parse(Json, Decoder) ->
    do_parse(Json, Decoder).
...
-spec nullable(gleam@option:option(EPX), fun((EPX) -> json())) -> json().
...
-spec array(list(EQB), fun((EQB) -> json())) -> json().
