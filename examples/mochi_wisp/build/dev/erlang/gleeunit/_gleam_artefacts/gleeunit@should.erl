...
-spec be_ok({ok, RDI} | {error, any()}) -> RDI.
be_ok(A) ->
    case A of
        {ok, Value} ->
            Value;
...
-spec be_error({ok, any()} | {error, RDN}) -> RDN.
be_error(A) ->
    case A of
        {error, Error} ->
            Error;
...
-spec be_some(gleam@option:option(RDQ)) -> RDQ.
be_some(A) ->
    case A of
        {some, Value} ->
            Value;
