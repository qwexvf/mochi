...
-opaque trie(HRQ, HRR) :: {trie,
        gleam@option:option(HRR),
        gleam@dict:dict(HRQ, trie(HRQ, HRR))}.
...
-spec do_delete(trie(HRZ, HSA), list(HRZ)) -> gleam@option:option(trie(HRZ, HSA)).
do_delete(Trie, Path) ->
    case {Path, Trie} of
        {[], {trie, _, Children_map}} ->
...
-spec fold(trie(HSH, HSI), HSL, fun((HSL, list(HSH), HSI) -> HSL)) -> HSL.
...
-spec get(trie(HST, HSU), list(HST)) -> {ok, HSU} | {error, nil}.
get(From, Path) ->
    case {Path, From} of
        {[], {trie, none, _}} ->
            {error, nil};
...
-spec has_path(trie(HTA, any()), list(HTA)) -> boolean().
has_path(Trie, Path) ->
...
-spec map(trie(HTQ, HTR), fun((HTR) -> HTU)) -> trie(HTQ, HTU).
map(Trie, Fun) ->
    {trie,
...
-spec delete(trie(HRS, HRT), list(HRS)) -> trie(HRS, HRT).
delete(Trie, Path) ->
...
-spec insert(trie(HTF, HTG), list(HTF), HTG) -> trie(HTF, HTG).
...
-spec from_list(list({list(HSN), HSP})) -> trie(HSN, HSP).
from_list(List) ->
    gleam@list:fold(
        List,
        new(),
...
-spec paths(trie(HUB, any())) -> list(list(HUB)).
paths(Trie) ->
...
-spec singleton(list(HUH), HUJ) -> trie(HUH, HUJ).
...
-spec subtrie(trie(HUQ, HUR), list(HUQ)) -> {ok, trie(HUQ, HUR)} | {error, nil}.
subtrie(Trie, Prefix) ->
    case {Prefix, Trie} of
        {[], _} ->
            {ok, Trie};
            ...
            case _pipe@2 of
                {ok, X@1} ->
                    {ok,
                        begin
                            _pipe@3 = maps:new(),
                            _pipe@4 = gleam@dict:insert(_pipe@3, First, X@1),
                            {trie, none, _pipe@4}
                        end};
...
-spec to_list(trie(HUZ, HVA)) -> list({list(HUZ), HVA}).
to_list(Trie) ->
...
-spec do_update(
    trie(HVO, HVP),
    list(HVO),
    fun((gleam@option:option(HVP)) -> gleam@option:option(HVP))
) -> gleam@option:option(trie(HVO, HVP)).
...
-spec update(
    trie(HVF, HVG),
    list(HVF),
    fun((gleam@option:option(HVG)) -> gleam@option:option(HVG))
) -> trie(HVF, HVG).
...
-spec values(trie(any(), HVZ)) -> list(HVZ).
values(Trie) ->
