...
-spec method_override(gleam@http@request:request(QDK)) -> gleam@http@request:request(QDK).
...
-spec sort_keys(list({binary(), QFB})) -> list({binary(), QFB}).
sort_keys(Pairs) ->
    gleam@list:sort(
        Pairs,
        fun(A, B) ->
...
-spec or_500({ok, QDV} | {error, any()}) -> {ok, QDV} |
    {error, gleam@http@response:response(body())}.
or_500(Result) ->
    case Result of
        {ok, Value} ->
...
-spec fn_with_bad_request_error(
    fun((QEJ) -> {ok, QEK} | {error, any()}),
    binary()
) -> fun((QEJ) -> {ok, QEK} | {error, gleam@http@response:response(body())}).
...
-spec multipart_body(
    ...
    integer(),
    integer(),
    fun((QED, bitstring()) -> {ok, QED} |
        {error, gleam@http@response:response(body())}),
    QED
) -> {ok, {gleam@option:option(buffered_reader()), integer(), QED}} |
    {error, gleam@http@response:response(body())}.
