...
export function serve_static(req, prefix, directory, handler) {
  ...
  if ($1 && $ instanceof $http.Get) {
    ...
    let path$1 = _block;
    let _block$1;
    let $2 = $uri.percent_decode(path$1);
    if ($2 instanceof Ok) {
      let p = $2[0];
      _block$1 = p;
    } else {
    ...
    _block$2 = $result.unwrap(_pipe$5, "");
    let file_type = _block$2;
    let mime_type = $marceau.extension_to_mime_type(file_type);
    let _block$3;
    if (mime_type === "application/json") {
      _block$3 = mime_type + "; charset=utf-8";
    } else if (mime_type.startsWith("text/")) {
      _block$3 = mime_type + "; charset=utf-8";
    } else {
      _block$3 = mime_type;
    }
    let content_type = _block$3;
    let $3 = $simplifile.file_info(path$2);
    if ($3 instanceof Ok) {
      let file_info = $3[0];
      let $4 = $simplifile.file_info_type(file_info);
      if ($4 instanceof $simplifile.File) {
        ...
        let _pipe$8 = $response.set_body(
          _pipe$7,
          new File(path$2, 0, new $option.None()),
        );
        let _pipe$9 = handle_etag(_pipe$8, req, file_info);
        return handle_file_range_header(_pipe$9, req, file_info, path$2);
      } else {
        return handler();
      }
    } else {
