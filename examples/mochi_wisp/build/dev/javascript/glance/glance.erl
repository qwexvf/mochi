...
-type definition(HNX) :: {definition, list(attribute()), HNX}.
...
-type bit_string_segment_option(HNY) :: bytes_option |
    int_option |
    float_option |
    bits_option |
    utf8_option |
...
-type record_update_field(HNZ) :: {record_update_field,
        binary(),
        gleam@option:option(HNZ)}.

-type variant_field() :: {labelled_variant_field, type(), binary()} |
    {unlabelled_variant_field, type()}.

-type field(HOA) :: {labelled_field, binary(), span(), HOA} |
    {shorthand_field, binary(), span()} |
    {unlabelled_field, HOA}.
...
-type parsed_list(HOB) :: {parsed_list,
        list(HOB),
        gleam@option:option(HOB),
        list({glexer@token:token(), glexer:position()}),
        integer()}.
...
-spec expect_name(
    list({glexer@token:token(), glexer:position()}),
    fun((binary(), list({glexer@token:token(), glexer:position()})) -> {ok, HOT} |
        {error, error()})
) -> {ok, HOT} | {error, error()}.
...
-spec until(
    ...
    HOY,
    list({glexer@token:token(), glexer:position()}),
    fun((HOY, list({glexer@token:token(), glexer:position()})) -> {ok,
            {HOY, list({glexer@token:token(), glexer:position()})}} |
        {error, error()})
) -> {ok, {HOY, integer(), list({glexer@token:token(), glexer:position()})}} |
    {error, error()}.
...
-spec list(
    fun((list({glexer@token:token(), glexer:position()})) -> {ok,
            {HUR, list({glexer@token:token(), glexer:position()})}} |
        {error, error()}),
    gleam@option:option(fun((span()) -> HUR)),
    list(HUR),
    list({glexer@token:token(), glexer:position()})
) -> {ok, parsed_list(HUR)} | {error, error()}.
...
-spec bit_string_segment_options(
    fun((list({glexer@token:token(), glexer:position()})) -> {ok,
            {HSO, list({glexer@token:token(), glexer:position()})}} |
        {error, error()}),
    list(bit_string_segment_option(HSO)),
    list({glexer@token:token(), glexer:position()})
) -> {ok,
        {list(bit_string_segment_option(HSO)),
            list({glexer@token:token(), glexer:position()})}} |
...
-spec optional_bit_string_segment_options(
    fun((list({glexer@token:token(), glexer:position()})) -> {ok,
            {HSH, list({glexer@token:token(), glexer:position()})}} |
        {error, error()}),
    list({glexer@token:token(), glexer:position()})
) -> {ok,
        {list(bit_string_segment_option(HSH)),
            list({glexer@token:token(), glexer:position()})}} |
    {error, error()}.
...
-spec bit_string_segment(
    fun((list({glexer@token:token(), glexer:position()})) -> {ok,
            {HSA, list({glexer@token:token(), glexer:position()})}} |
        {error, error()}),
    list({glexer@token:token(), glexer:position()})
) -> {ok,
        {{HSA, list(bit_string_segment_option(HSA))},
            list({glexer@token:token(), glexer:position()})}} |
    {error, error()}.
...
-spec delimited(
    ...
    list({glexer@token:token(), glexer:position()}),
    fun((list({glexer@token:token(), glexer:position()})) -> {ok,
            {HUH, list({glexer@token:token(), glexer:position()})}} |
        {error, error()}),
    glexer@token:token()
) -> {ok, {list(HUH), list({glexer@token:token(), glexer:position()})}} |
    {error, error()}.
...
-spec comma_delimited(
    ...
    list({glexer@token:token(), glexer:position()}),
    fun((list({glexer@token:token(), glexer:position()})) -> {ok,
            {HVJ, list({glexer@token:token(), glexer:position()})}} |
        {error, error()}),
    glexer@token:token()
) -> {ok,
        {list(HVJ), integer(), list({glexer@token:token(), glexer:position()})}} |
    {error, error()}.
...
-spec field(
    list({glexer@token:token(), glexer:position()}),
    fun((list({glexer@token:token(), glexer:position()})) -> {ok,
            {HWU, list({glexer@token:token(), glexer:position()})}} |
        {error, error()})
) -> {ok, {field(HWU), list({glexer@token:token(), glexer:position()})}} |
    {error, error()}.
field(Tokens, Parser) ->
    case Tokens of
