...
-spec push_key({ok, EBV} | {error, get_error()}, binary()) -> {ok, EBV} |
    {error, get_error()}.
push_key(Result, Key) ->
    case Result of
        {ok, T} ->
...
-spec expect_end_of_line(
    list(binary()),
    fun((list(binary())) -> {ok, {EEA, list(binary())}} | {error, parse_error()})
) -> {ok, {EEA, list(binary())}} | {error, parse_error()}.
...
-spec do(
    {ok, {EEL, list(binary())}} | {error, parse_error()},
    fun((EEL, list(binary())) -> {ok, EEO} | {error, parse_error()})
) -> {ok, EEO} | {error, parse_error()}.
do(Result, Next) ->
    case Result of
        {ok, {A, Input}} ->
            Next(A, Input);
...
-spec expect(
    list(binary()),
    binary(),
    fun((list(binary())) -> {ok, {EET, list(binary())}} | {error, parse_error()})
) -> {ok, {EET, list(binary())}} | {error, parse_error()}.
