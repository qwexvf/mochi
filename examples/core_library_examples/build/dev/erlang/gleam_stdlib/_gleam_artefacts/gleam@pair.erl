...
-spec map_first({CLC, CLD}, fun((CLC) -> CLE)) -> {CLE, CLD}.
map_first(Pair, Fun) ->
    {A, B} = Pair,
    {Fun(A), B}.
...
-spec map_second({CLF, CLG}, fun((CLG) -> CLH)) -> {CLF, CLH}.
map_second(Pair, Fun) ->
    {A, B} = Pair,
    {A, Fun(B)}.
