...
pub type SchemaEvent {
  ...
  SchemaDataLoaderBatch(loader_name: String, batch_size: Int, duration_ns: Int)
}
  ...
  case get_data_loader(context, loader_name) {
    Ok(loader) -> {
      ...
      case context.telemetry_fn {
        Some(fn_) -> fn_(SchemaDataLoaderBatch(loader_name, 1, duration_ns))
        None -> Nil
      }
...
// Guards
// ============================================================================
// Guards run before the resolver and can short-circuit with an error.
// If the guard returns Ok(Nil), the resolver proceeds normally.
// If the guard returns Error(message), the resolver is skipped.
  ...
  case field.resolver {
    Some(current_resolver) -> {
      let guarded = fn(info: ResolverInfo) {
        use _ <- result.try(guard_fn(info))
        current_resolver(info)
      }
      FieldDefinition(..field, resolver: Some(guarded))
    }
    None -> field
  }
}
