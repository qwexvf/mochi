...
pub fn new_with_options_test() {
  let opts = dataloader.DataLoaderOptions(max_batch_size: 50, cache_enabled: False)
  let loader =
...
pub fn string_batch_loader_test() {
  ...
  let #(_loader, result) = dataloader.load(loader, dataloader.string_key("test"))
  should.be_true(result.is_ok(result))
}
...
pub fn cache_disabled_test() {
  let opts = dataloader.DataLoaderOptions(max_batch_size: 100, cache_enabled: False)
  let loader =
    dataloader.new_with_options(
